 ...
 */
//从这里判断bio是否是sync的条件可以看出
//read和带sync标志的IO才是同步的
//write是异步的
 ...
 *
 * blk_plug用来建立一个queue来存储一个零散的I/O一段时间.它会merge一些顺序的
 * 请求到一个单独的大的request中去. 
 *
 ...
 * 当我们添加一个request到一个plug list或者我们试图做merge的时候，不用disable
 * 抢占，因为blk_schedule_flush_list只在task sleep的时候flush plug list，具体
 * 可以看 sched_submit_work 中会判断task是否是sleep状态.
 */
struct blk_plug {
