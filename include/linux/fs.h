...
//wisen: 内核对页缓存的基本操作包含了在一个页缓存所形成的radix树中查找，增加和删除一个
//页缓存。基于radix的基本操作函数，页高速缓存的处理函数如下：
...
struct address_space {
	...
	//wisen: address_space结构是页高速缓存机制中的核心数据结构，该结构并不是对某一个
	//页高速缓存进行描述，而是以页高速缓存的所有者（owner）为单位，对其所拥有的缓存
	//进行抽象描述。页高速缓存中每个页包含的数据肯定属于某个文件，该文件对应的inode对象
	//就称为页高速缓存的所有者。
	//页缓存与文件系统和内存管理都有联系。每个inode结构中都嵌套一个address_space结构，
	//即inode字段中的i_data；同时inode中还有i_maping字段指向所嵌套address_spaces结构。
...
static inline loff_t i_size_read(const struct inode *inode)
{
	...
	return i_size;
#elif BITS_PER_LONG==32 && 1/*defined()*/
	loff_t i_size;
