			    ...
			    struct bio *bio)
{
	...
	//从这里看出io的prio有两种，bio的prio和request_queue的prio
	//在merge过程中会选择bio和request_queue中最优的prio
			    ...
			    unsigned int *request_count)
{
	...
	*request_count = 0;
	...
	list_for_each_entry_reverse(rq, plug_list, queuelist) {
		...
		//真正merge的地方，但是怎么算的还得搞清楚
		//这里merge其实还是在进程内部的plug
		//这里的merge就是2个方向，插到后面还是前面，或者无法merge
...
//根据bio标示初始化request,也就是说request是从bio构建的
...
void generic_make_request(struct bio *bio)
{
	 ...
	 */
	//wisen: 如果当前进程有bio_list，就将bio加入到当前进程的bio_list中
	//wisen: 如果没有就往下面走，创建一个bio_list
	if (current->bio_list) {
...
void submit_bio(int rw, struct bio *bio)
{
	...
	if (bio_has_data(bio)) {
		...
		//因为一个sector大小是512，所以这里的count都是计算的
		//sector的个数，所以右移9
		...
		//统计PGPGIN/PGPGOUT事件
		if (rw & WRITE) {
			//write在 submit_page_section 中做了统计
			count_vm_events(PGPGOUT, count);
		} else {
			//这里累计进程的read bytes, 为什么write不统计，还是write在别的地方统计?
			//这里做统计的时候需要开启CONFIG_TASK_IO_ACCOUNTING
			//也就是一个简单的累加，所以开启这个宏对系统影响不大
		...
		//可以通过开启block_dump来debug IO
		//在这里我们还是能看到IO来自己那个进程
		//可以看到IO是read/write, IO的大小，IO要写到那个磁盘，sector是多少
 ...
 *     %true  - still buffers pending for this request
 **/
 //wisen: blk_end_request()一般把io_schedule()后放弃CPU的进程唤醒。
