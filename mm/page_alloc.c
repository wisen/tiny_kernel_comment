...
#if 1//#if 1//#ifdef CONFIG_HIGHMEM//CONFIG_HIGHMEM=y
	[N_HIGH_MEMORY] = { { [0] = 1UL } },
#endif
...
#if 0//#ifdef CONFIG_ZONE_DMA
	 256,
#endif
#if 0//#ifdef CONFIG_ZONE_DMA32
	 256,
#endif
#if 1//#if 1//#ifdef CONFIG_HIGHMEM//CONFIG_HIGHMEM=y
	 32,
...
#if 0//#ifdef CONFIG_HAVE_MEMBLOCK_NODE_MAP
 ...
 * -- nyc
 */
 //wisen: 分裂函数的实现也是显而易见的，它完全遵照伙伴算法的分裂原理。
 //wisen: 这里有两个分配阶，一个是申请页框时指定的low，一个是在上级函数中遍历时
 //wisen: 所选定的high。该函数从high分配阶开始递减向low遍历，也就是从较大的页框块
 //wisen: 开始依次分裂。
 //wisen: 比如high为4，而low为2。那么第一遍历时，将大小为16（分配阶为4）的页框块一份为2。
 //wisen: 通过list_add()将后面的8个连续页框块加入下级链表（分配阶为3），下级链表通过
 //wisen: 将area指针自减即可得到，后8个页框块的指针也通过page+size获得，而page仍然
 //wisen: 指向最初的页框块首页框。此时还要对分配阶为3的链表更新nr_free，以及通过
 //wisen: set_page_order()对后8个页框块设置一些标志。
 //wisen: 第二次遍历将前面8个页框块继续一分为二，将后4个页框块加入area所指向的
 //wisen: 下级链表（分配阶为2）。第三次遍历时，循环条件已经不再满足，因此返回前4个
 //wisen: 页框块首页框的描述符地址page。
 ...
 */
 //wisen: 该函数的实现比较简单，从当前指定的分配阶到最高分配阶依次进行遍历。
 //wisen: 在每次遍历的分配阶链表中，根据参数migratetype选择正确的迁移队列。
 //wisen: 根据以上的限定条件，当选定一个页框块链表后，只要该链表不为空，
 //wisen: 就说明可以分配该分配阶对应的页框块。
 //wisen: 一旦选定在当前遍历的分配阶链表上分配页框，那么就通过list_entry（）
 //wisen: 将该页框块从链表上移除。然后将页框块首页框的PG_buddy标志删除，
 //wisen: 删除该标志说明当前页框块已经不属于伙伴链表。并且将该首页框描述符中的private
 //wisen: 置0，该字段中本来保存的是其所处页框块的分配阶。以上这个过程通过
 //wisen: rmv_page_order()完成。此外，还要更新页框块链表nr_free的值。
 //wisen: __rmqueue_smallest()内部还有一个重要的函数expand()。
 //wisen: 进入该函数的条件是当所申请的分配阶order小于当前选中的分配阶current_order，
 //wisen: 也就是说指定的分配阶链表中没有空闲的页框块，只能选用较大的页框块。因此，
 //wisen: expand()必须按照伙伴算法的分裂原理将比较大的页框块分割成较小的块。
static inline
...
#if 0//#ifdef CONFIG_MEMORY_ISOLATION
 ...
 */
 //wisen: 传递到此函数中的zone表示伙伴算法将从该内存管理区中分配页框，order即分配阶，
 //wisen: migratetype表示迁移类型。该函数首选__rmqueue_smallest()进行内存分配，
 //wisen: 如果在指定的迁移类型上分配失败后，再选用其他备用的迁移列表进行内存分配，
 //wisen: 该过程通过__rmqueue_fallback()完成。总之内核总是在竭尽全力保证满足分配内存的
 //wisen: 请求
...
again:
	//wisen: 这里buffered_rmqueue并没有进行真正的alloc动作
	//wisen: 它先判断,如果order等于0,那么就直接启动per-CPU机制来分配
	//wisen: 否则调用__rmqueue来分配
	if (likely(order == 0)) {
		struct per_cpu_pages *pcp;
		struct list_head *list;
...
#ifdef CONFIG_FAIL_PAGE_ALLOC//not set
...
static bool __zone_watermark_ok(struct zone *z, unsigned int order,
			...
			long free_pages)//wisen: 假如这里free_pages=10 order=2
{
	...
	free_pages -= (1 << order) - 1;//wisen: free_pages=7
	...
	for (o = 0; o < order; o++) {
		...
		min >>= min_free_order_shift;//wisen: 目前cat的值是4 /proc/sys/vm/min_free_order_shift
...
{
#if 0//#ifdef CONFIG_ZONE_MOVABLE_CMA//CONFIG_ZONE_MOVABLE_CMA is not set//=no
...
#else
	int alloc_flags = ALLOC_WMARK_LOW|ALLOC_CPUSET|ALLOC_FAIR;//wisen: =0x141
#endif
...
retry_cpuset:
	...
	page = get_page_from_freelist(gfp_mask|__GFP_HARDWALL, nodemask, order,
			...
			preferred_zone, classzone_idx, migratetype);
	if (unlikely(!page)) {//wisen: 如果从freelist分配失败,那么就尝试用__alloc_pages_slowpath来分配
					//wisen: 但是在__alloc_pages_slowpath中可能会有block io, error handling
					//wisen: 包括wakeup kswapd先回收page，还有banlance的操作，所以比较慢
		/*
...
unsigned long __get_free_pages(gfp_t gfp_mask, unsigned int order)
{
	 ...
	 * a highmem page//wisen:__get_free_pages 返回一个32位地址，但是这不代表一个highmem page
	 */
...
{
	set_zonelist_order();
	//wisen: 根据system_state查看系统的运行状态，在内核启动阶段，它的值保持为0
	//wisen: 也即SYSTEM_BOOTING，只有在start_kernel执行到最后一个函数rest_init后
	//wisen: 才会进入SYSTEM_RUNNING阶段。如果为内核启动阶段，那么调用
	//wisen: __build_all_zonelists初始化分配器管理区链表，否则挂起系统，显然内存管理功能初始化出现异常为致命错误。
	if (system_state == SYSTEM_BOOTING) {
		__build_all_zonelists(NULL);
		mminit_verify_zonelist();
		cpuset_init_current_mems_allowed();
	} else {
#if 0//#ifdef CONFIG_MEMORY_HOTPLUG
		if (zone)
		...
		unsigned long start_pfn, enum memmap_context context)
{
	...
	for (pfn = start_pfn; pfn < end_pfn; pfn++) {
		...
		INIT_LIST_HEAD(&page->lru);
#if 0//#ifdef WANT_PAGE_VIRTUAL//WANT_PAGE_VIRTUAL is not set
						...
						unsigned long *zholes_size)
{
	...
	//wisen debug: x559 16+1
	printk("wisen: realtotalpages=%d\n", realtotalpages);
	//wisen: realtotalpages=258816
	for (i = 0; i < MAX_NR_ZONES; i++)
		realtotalpages -=
	...
	printk("On node %d totalpages: %lu\n", pgdat->node_id,
							realtotalpages);
	//On node 0 totalpages: 252160
}

#if 1//#ifndef CONFIG_SPARSEMEM
/*
...
#if defined(CONFIG_MTK_MEMCFG) && 1/*defined()*/
		mem_map_size = size;
#endif
	}
