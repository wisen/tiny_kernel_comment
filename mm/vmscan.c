		 ...
		 unsigned long nr_pages_scanned, unsigned long lru_pages)
{
	...
	while (total_scan >= batch_size ||
		...
		if (ret == SHRINK_STOP) {
			printk(KERN_ERR, "wisen SHRINK_STOP");
			break;
		}
			 ...
			 struct scan_control *sc)
{
	...
	if (clear_page_dirty_for_io(page)) {
		...
		//wisen 这里是最关键的地方
		//wisen: 如果你是normal的io:
		//wisen: 比如: f2fs_meta_aops --> f2fs_write_meta_page
		//wisen: 如果你是zram page：
		//a_ops->writepage ----> swap_aops.writepage == swap_writepage

		//wisen: 不同的mapping对应不同的a_ops
				      ...
				      bool force_reclaim)
{
	...
	while (!list_empty(page_list)) {
		 ...
		 */
		 //wisen: 如果是匿名页并且没有被置位成page swap, 那么就添加到swap中
...
activate_locked:
		...
		//wisen: 如果添加到swap space失败了, 说明它不是一个swapping的候选者，把给它分配的swap space释放掉
		//wisen: 并且将这个设置active标志，并增加pgactive计数
		     ...
		     struct scan_control *sc, enum lru_list lru)
{
	...
	if (global_reclaim(sc)) {
		...
		//wisen: 如果当前是kswapd线程，那么就将pgscan_kswapd_normal加1
		//如果不是kswapd线程，那么就将pgscan_direct_normal加1
		if (current_is_kswapd())
 ...
 */
 //wisen: file LRU list的更新策略
 //系统要求inactive file-cache lru上页的总量不低于active file-cache lru上页的总量即可。
 //当内存回收启动时发现不满足上述情况，则从active file-cache lru链表的尾部拿一些page，清空ACCESSED标记，
 //保持PG_referenced，放到inactive的头部。然后扫描inactive的尾部，并进行以下处理：
//如果是PG_referenced为1，清0，不管ACCESSSED标记是什么，都放到active file-cache lru的头。
...
//如果是PG_referenced为1，清0，不管ACCESSSED标记是什么，都放到active file-cache lru的头。
//如果映射了该文件页的进程的PTE中有ACCESSED标记为1，则放到active file-cache lru的头。否则，则写回并释放。
//wisen: anon LRU list的更新策略
//系统要求inactive anon lru的总量不能低于某个值。该值是一个经验值。对于1G的系统，
//要求inactive anon lru上挂的内存页总量不低于250M。当内存回收启动时发现inactive anon不足时，
//则从active anon lru list尾部拿一些page（一般为32个），将他们的PTE中的ACCESSD标记清0（每次访问这个页面，
//硬件会将该位置1），放在inactive list的链表头。然后遍历inactive lru的链表尾部，如果此时ACCESSD的标记为1（说明
//最近被访问过），则重新放到active list中，否则将交换出去。
 ...
 * wisen: 决定如何积极的去扫描file LRU和anon LRU.
 *
			   ...
			   struct scan_control *sc, unsigned long *nr)
{
	...
	//wisen: 如果没有swap空间了，就不要扫描anon pages，就只扫描file
	if (!sc->may_swap ||
	...
	if (global_reclaim(sc)) {
		...
		zonefile = zone_page_state(zone, NR_ACTIVE_FILE) +
             ...
             //wisen: 当file和free之和小于high water mark的时候，就只扫描anon
	 ...
	 */
      //wisen: 当inactive file 大于 active file的时候，就只扫描file
	 ...
	 */
	 //wisen: swappiness是100的时候，anon和file有相同的优先级
	...
	if (vmscan_swap_file_ratio) {
	//wisen: 如果开启zram,并且vmscan_swap_file_ratio=1
	//wisen: 那么anon和file的优先级就简单的根据他们的多少来计算
	//wisen: anon多那么anon的优先级就高，file多那么file的优先级就高
...
out:
	...
	for (pass = 0; !some_scanned && pass < 2; pass++) {
		for_each_evictable_lru(lru) {
			...
			//wisen: sc->priority默认是DEF_PRIORITY=12
			//wisen: 也就是扫描的大小是size除以4096
			scan = size >> sc->priority;

                    //wisen: 如果size很小，小于4096大于32，那么扫描的大小是SWAP_CLUSTER_MAX=32
                    //wisen: 如果size很小，小到小于32，那么扫描的大小就是size本身
...
#if 0//#ifdef CONFIG_ZONE_MOVABLE_CMA//CONFIG_ZONE_MOVABLE_CMA is not set
/*
 ...
 */
 //wisen: shrink_lruvec是基本的per-zone的page freer.kswapd和direct reclaim都会走到这.
 ...
 * 回收的时候做内存压缩是为了满足大块内存(high-order)分配请求.
 * It reclaims order-0 pages before compacting the zone.
 * 它在做内存压缩前会先回收0-order的pages
 * should_continue_reclaim() returns true if more pages should be reclaimed such
 * that when the page allocator calls try_to_compact_zone() that it will have 
 * enough free pages to succeed.
 * should_continue_reclaim返回true代表得继续回收更多的页，这样下次页分配器调用
 * try_to_compact_zone()的时候就有足够多的页用来压缩并压缩成功.
 * It will give up earlier than that if there is difficulty reclaiming pages.
 * 
 */
 //wisen: compaction压紧,可理解为整理内存碎片.
					...
					struct scan_control *sc)
{
	 ...
	 */
	 //wisen: 如果我们没有回收足够的页来做内存压缩,并且inactive lists也足够大，
	 //wisen: 我们就继续回收
					  ...
					  struct scan_control *sc)
{
	...
	if (global_reclaim(sc))
		count_vm_event(ALLOCSTALL);//wisen: 这个就是统计vmstat中的allocstall值
 ...
 * the high watermark.
 * wisen: kswapd shrinks的目的是要满足high watermark的需求.
 *
 * Returns true if kswapd scanned at least the requested number of pages to
 * reclaim or if the lack of progress was due to pages under writeback.
 * This is used to determine if the scanning priority needs to be raised.
 * wisen: kswapd_shrink_zone在达到回收目标后或者缺乏进展因为页处于writeback状态
 * wisen: 后返回.之后kswapd会决定是否需要提高扫描的等级.
 */
 ...
 * wisen: balance_pgdat会扫描所有node中的所有zones，直到所有的zones都能达到
 * wisen: high water mark.
 *
 * Returns the final order kswapd was reclaiming at
 *
 * There is special handling here for zones which are full of pinned pages.
 //wisen: 这里有个对那些充满被占用的pages的zones的特殊处理.
 * This can happen if the pages are all mlocked, or if they are all used by
 * device drivers (say, ZONE_DMA).  Or if they are all in use by hugetlb.
 //wisen: pinned pages发生情况有：page被mlock了,或者被drives占用,或者被hugetlb使用
 * What we do is to detect the case where all pages in the zone have been
 * scanned twice and there has been zero successful reclaim.  Mark the zone as
 * dead and from now on, only perform a short scan.  Basically we're polling
 * the zone for when the problem goes away.
 *
 //wisen: kswapd是按照highmem->normal->dma顺序来扫描zones的.
 //wisen: kswapd会跳过那些free pages数目大于high water mark的zone的，但是一旦
 //wisen: 发现这个zone的free pages小于high water mark,就会立即扫描这个zone,不管
 //wisen: 下个等级的zone中的free pages数量处于什么level.
 //wisen: 这种页分配回退机制可以确保zone中的老化页的平衡
							...
							int *classzone_idx)
{
	...
	do {
		 ...
		 */
		 //wisen: 如果kswapd_shrink_zone没有回收足够的pages，那么sc.priority--
		 //wisen: sc.priority--就意味着下次scan的page数量多一倍
		 //wisen: scan pages=totol_size>>sc.priority
...
static void kswapd_try_to_sleep(pg_data_t *pgdat, int order, int classzone_idx)
{
	...
	if (prepare_kswapd_sleep(pgdat, order, remaining, classzone_idx)) {
		//wisen: HZ=100/*CONFIG_HZ=100*/=250
...
static int kswapd(void *p)
{
	 ...
	 * wisen: kswapd一般情况下不需要memory，但是有时候会需要一点点memory来做page out.
	 */
 ...
 *
 */
 //wisen: evictable被驱逐的，从LRU list中可被驱逐，也就是可回收?
 //wisen: 判断页是否可以回收?
 //wisen: 一个页不能被回收有2点:
 //wisen: page被标记为不可回收
 //wisen: page是mlocked VMA的一部分
