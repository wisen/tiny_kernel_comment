		...
		struct iov_iter *iter, ssize_t written)
{
	...
	//这里的ppos实际上就是 file中的 file->f_pos;
	//比如我上次读到4097字节位置，那么这里的index就为1
	...
	for (;;) {
		...
		cond_resched();
find_page:
		//先找是否存在这个index的page cache
		page = find_get_page(mapping, index);
		if (!page) {
			//如果没有就调用page_cache_sync_readahead(同步预读)来分配一个page
...
readpage:
		...
		//这里真正的开始读
		//这里的mapping->a_ops->readpage是调用的具体的文件系统中注册的readpage: f2fs_read_data_page
		//最终调用 mpage_readpage
...
generic_file_read_iter(struct kiocb *iocb, struct iov_iter *iter)
{
	...
	//如果应用层加了O_DIRECT标志, 直接走这里，go direct to bio
...
generic_file_direct_write(struct kiocb *iocb, struct iov_iter *from, loff_t pos)
{
	...
	data = *from;
	//在 f2fs_iget 中初始化inode->i_mapping->a_ops = &f2fs_dblock_aops;
	//这里的mapping->a_ops->direct_IO是调用的具体的文件系统中注册的direct_IO: f2fs_direct_IO
