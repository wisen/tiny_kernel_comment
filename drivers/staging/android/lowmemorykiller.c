...
{
	trace_mm_lowmemorykiller_scan_start(0);
...
//wisen start
//wisen: 这里的解法是为了解决X572O上后台缓存大量进程后，file page保持在一个高的水位
//wisen: 最终导致lmk的阈值无法达到，lmk不被触发，进程越开越多，最后swapuse达到1G左右
//wisen: 系统卡死. 期间尝试了调整swapiness，但是收效甚微. 最后选择在这里检查swapuse,
//wisen: 如果达到total_swap_pages的60%，就触发lmk，收到了一定的效果.但是这里有个缺陷
//wisen: 那就是adj的范围不够大
//wisen: 解决这个问题，我们最后选择了Android Go上的lmk机制，就是打开memcg后，计算pressure
//wisen: 通过pressure来触发lmk，miedum的时候adj是800.
unsigned long swap_used = 0;
int swap_press = 0;
swap_used = total_swap_pages - atomic_long_read(&nr_swap_pages);
if (swap_used * 10 > total_swap_pages * 6) {
	swap_press = 1;
}
//wisen end
	...
	for (i = 0; i < array_size; i++) {
		...
		if ((other_free < minfree && other_file < minfree) || swap_press) {
#if 1//#ifdef //
	...
	lowmem_print(1, "wisen selected_oom_score_adj=%d\n",selected_oom_score_adj);
...
log_again:
		...
		*/
		...
		if (oom_score_adj < min_score_adj) {//wisen 从这里跳出
			task_unlock(p);
			continue;
		}

#ifndef CONFIG_MT_ENG_BUILD//=y
		tasksize = get_mm_rss(p->mm);
#ifdef CONFIG_ZRAM
		tasksize += get_mm_counter(p->mm, MM_SWAPENTS);
#endif
...
#ifdef CONFIG_MTK_GMO_RAM_OPTIMIZE//=y
		/*
		...
		*/
		...
		lowmem_print(1, "wisen select '%s' (%d), adj %d, score_adj %hd, size %d, to kill\n",
			     p->comm, p->pid, REVERT_ADJ(oom_score_adj), oom_score_adj, tasksize);
	}

	//wisen add start
	clock_t c_end = clock();
	lowmem_print(1, "wisen loop consume time:%ld\n", c_end-c_start);
      //wisen add end
#ifdef CONFIG_MT_ENG_BUILD
	if (log_offset > 0)
		lowmem_print(1, "\n%s", lmk_log_buf);
	...
	rcu_read_unlock();
	spin_unlock(&lowmem_shrink_lock);
	trace_mm_lowmemorykiller_scan_end(0);
	return rem;
}
...
static int lowmem_adj_array_set(const char *val, const struct kernel_param *kp)
{
	...
	ret = param_array_ops.set(val, kp);
       lowmem_print(1, "wisen: kp->name=%s, pid=%d\n", kp->name, current->pid);
...
static int lowmem_minfree_array_set(const char *val, const struct kernel_param *kp)
{
	...
	ret = param_array_ops.set(val, kp);
       lowmem_print(1, "wisen: minfree debug[%d][%s][%s]\n", current->pid, current->comm, val);
...
static const struct kparam_array __param_arr_minfree = {
...
module_param_cb(minfree, &lowmem_minfree_array_ops,
		.arr = &__param_arr_minfree, S_IRUGO | S_IWUSR);
__MODULE_PARM_TYPE(minfree, "array of uint");
#else
