...
static struct pinctrl_state *avdd_output0,*avdd_output1;
#if TOUCH_FILTER
...
static char final_ctp_firmware_version[16] = "1050";
...
extern int ltr559_get_psval(void);
...
int gesture_enabled = 0;//for debug  0-->1
int gesturechar_enabled =0;//for debug   0-->1
...
//add by rulei.zhou for firmware information
...
static DEVICE_ATTR(tp_final_firmware_version, 0444, tp_final_fw_version_show, NULL);
// add end
...
void gtp_power_on_dts(int enable)
{
	
	printk("[tpd]tpd_gpio_output enable = %d\n", enable);
		
		if (enable){
			pinctrl_select_state(pinctrl1, avdd_output1);
			printk("++++++++++enable=========1");
		}
		else
			pinctrl_select_state(pinctrl1, avdd_output0);
	
}

int gtp_get_gpio_info(void)
{
	int ret;
	printk("[tpd ] mt_tpd_pinctrl+++++++++++++++++\n");
	/* pinctrl = devm_pinctrl_get(&pdev->dev); */
	avdd_output0 = pinctrl_lookup_state(pinctrl1, "state_avdd_output0");
	if (IS_ERR(avdd_output0)) {
		ret = PTR_ERR(avdd_output0);
		GTP_ERROR("fwq Cannot find touch pinctrl state_avdd_output0!\n");
		return ret;
	}
	avdd_output1 = pinctrl_lookup_state(pinctrl1, "state_avdd_output1");
	if (IS_ERR(avdd_output1)) {
		ret = PTR_ERR(avdd_output1);
		GTP_ERROR("fwq Cannot find touch pinctrl state_avdd_output1!\n");
		return ret;
	}
	TPD_DEBUG("[tpd] mt_tpd_pinctrl----------\n");
	return 0;
}
...
static int tpd_irq_registration(void)
{
	...
	GTP_INFO("[%s]irq:%d, debounce:%d-%d: gtp_eint_trigger_type :%d\n", __func__, touch_irq, ints[0], ints[1],gtp_eint_trigger_type);
	return ret;
}
#endif
	...
	do {
		...
		if (DOZE_ENABLED == doze_status) {
			...
			if (ret > 0) {
				...
				if(doze_buf[2]!=0){
					if (!QUERYBIT(gestures_flag, doze_buf[2])||(!gesturechar_enabled && doze_buf[2]>='a'&&doze_buf[2]<='z')||(gesturechar_enabled && doze_buf[2]>='a'&&doze_buf[2]<='z' &&!(tpgesture_area_w>320&&tpgesture_area_h>320))) {
						printk("GTP-gesture, this gesture has been disabled.");
						doze_buf[2] = 0x00;
					...
					if(get_psval) {
						psstatus=get_psval();
						GTP_DEBUG("psstatus = %d\n",psstatus);
						if(psstatus == 1){
							//ps is closed , so don't report any key.\r	686
							printk("GTP-gesture wakeup by tp ,but ps value cover by something \n");
						}
					}
					gesture_data.data[0] = doze_buf[2];	// gesture type
					...
					if(psstatus == 0){
						...
						input_sync(tpd->dev);
						GTP_DEBUG("GTP-gesture report key");
						// clear 0x814B
						doze_buf[2] = 0x00;
						gtp_i2c_write(i2c_client_point, doze_buf, 3);
						//gtp_irq_enable();
					...
					// clear 0x814B
					GTP_INFO("gtp_i2c_read read nothing");
					doze_buf[2] = 0x00;
					gtp_i2c_write(i2c_client_point, doze_buf, 3);
					gtp_enter_doze(i2c_client_point);
					//gtp_irq_enable();
...
#endif
				GTP_DEBUG("wuhao  %d)(%d, %d)[%d]", id, input_x, input_y, input_w);
				//printk("wuhao  %d)(%d, %d)[%d]", id, input_x, input_y, input_w);
