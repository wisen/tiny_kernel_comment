...
int i2c_himax_read(struct i2c_client *client, uint8_t command, uint8_t *data, uint8_t length, uint8_t toRetry)
{
	...
	if((NULL!=client) && (length>0) && (length<=128))
	{
		// DMA Write
		memcpy(gpDMABuf_va, &command, 1);
		client->addr = (client->addr & I2C_MASK_FLAG) | I2C_DMA_FLAG;
		if((ret=i2c_master_send(client, (unsigned char *)gpDMABuf_pa, 1))!=1)
...

	{
		...
		ret = i2c_master_recv(client, (unsigned char *)gpDMABuf_pa, length);
