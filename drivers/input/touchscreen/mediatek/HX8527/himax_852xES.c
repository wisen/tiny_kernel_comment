...
static char final_ctp_firmware_version[16] = "0x03";
...
int GestureTranslate(int gesture_id);
#if 1//#ifdef CONFIG_OF//CONFIG_OF=y
...
static ssize_t tp_firmware_version_show(struct device *dev,struct device_attribute *attr,char *buf)
{
	...
	char data[12]= {0};
	struct i2c_client *client = hx_i2c_client_point;
	if (i2c_himax_read(client, 0x39, data, 1, 3) < 0)
		num_read_chars = sprintf(buf,"get tp fw version fail!\n");
	else
		num_read_chars = sprintf(buf, "0x%02X\n", data[0]);
...
static struct device_attribute *himax852xes_attrs[] = {
...
static int i_update_FW(void)
{
	...
	if (1)/*(( private_ts->vendor_fw_ver_H < i_CTPM_FW[FW_VER_MAJ_FLASH_ADDR] )
...
static int himax_loadSensorConfig(struct i2c_client *client, struct himax_i2c_platform_data *pdata)
{
	...
	int rc= 0;
#endif
#if 0//#ifndef CONFIG_OF//CONFIG_OF=y
#if defined(HX_LOADIN_CONFIG)
	int i = 0;
#endif
#endif
			...
			if(vk_press == 0x00)
				{
					...
					finger_on = 1;
					AA_press = 1;
					I("finger_num:%d,finger_pressed:%d",finger_num,finger_pressed);
...
#if 1//#ifdef CONFIG_OF//CONFIG_OF=y_TOUCH
...
static int touch_event_handler(void *ptr)
{
	...
	int ret_event = 0;
#endif
	struct timespec timeStart={0}, timeEnd={0}, timeDelta={0};
...
if(gesture_data.data[0]==0xaa || gesture_data.data[0]==0xab || gesture_data.data[0]==0xba ||gesture_data.data[0]==0xbb )
	ret=sprintf(buff,"%x",gesture_data.data[0]);
if(gesture_data.data[0]==0xcc)buff[0]='f';
ret = simple_read_from_buffer(page, size, ppos, buff, 1);
return ret;
	
}*/
static ssize_t ht1x_gesture_data_read(struct file *file, char * buffer, size_t size, loff_t * ppos)
{
	int len,ret = -1;
	char *page = NULL;
	char *ptr = NULL;
	...
	if(gesture_data.data[0]==0xcc)
		ptr += sprintf(ptr,"%c\n",'f');
	else if(gesture_data.data[0]==0xaa || gesture_data.data[0]==0xab || gesture_data.data[0]==0xba ||gesture_data.data[0]==0xbb )
		ptr += sprintf(ptr,"%x\n",gesture_data.data[0]);
	else
		ptr += sprintf(ptr,"%c\n",gesture_data.data[0]);
	len = ptr - page;
	...
	*ppos += len;
	E("CTP gesture ret:%d ppos:%d ges:%x char:%c\n",ret,(int)*ppos,gesture_data.data[0],*ptr);
	if(ret)
	{
		kfree(page);
		return ret;
...
static ssize_t ht1x_gesture_data_write(struct file *filp, const char __user * buf, size_t len, loff_t * off)
{
	int tempchar3=0;
	...
	{
		printk("ht1x-gesture bad command format 0:%c 1:%c 2:%c!!!",buff[0],buff[1],buff[2]);
		return -EPERM;
	}
	...
	printk("ht1x-gesture buff 0:%c 1:%c 2:%c!!!",buff[0],buff[1],buff[2]);
...
static const struct file_operations ht1x_fops = {
	.owner = THIS_MODULE,
	.read = ht1x_gesture_data_read,
	.write = ht1x_gesture_data_write,
};
...
int Htp_enter_doze(struct i2c_client *hx_i2c_client_point){
	int ret;
	uint8_t buf[2] = {0};
	gesture_enabled=((gesturechar_enabled)||(gesturedouble_enabled));
	gesture_clear_wakeup_data();
	...
	if(gesture_enabled == 0){
		I("Disable GESTURE function \n");
		return -1;
	}
	else{
		buf[0] = 0x8F;
		buf[1] = 0x20;
		ret = i2c_himax_master_write(hx_i2c_client_point, buf, 2, DEFAULT_RETRY_CNT);
		if (ret < 0)
		{
			E("[himax] %s: I2C access failed addr = 0x%x\n", __func__, hx_i2c_client_point->addr);
			return ret;
		}
...
int Htp_leave_doze(struct i2c_client *hx_i2c_client_point){
	int ret;
	uint8_t buf[2] = {0};

	if (gesture_enabled){
		i2c_himax_write_command(hx_i2c_client_point, HX_CMD_TSSOFF, DEFAULT_RETRY_CNT);
		msleep(40);
		//Sleep in
		i2c_himax_write_command(hx_i2c_client_point, HX_CMD_TSSLPIN, DEFAULT_RETRY_CNT);
		I("gaoyuan!GESTURE Success!\n");
		msleep(50);
		I("gaoyuan!leave doze!\n");
		buf[0] = 0x8F;
		buf[1] = 0x00;
		ret = i2c_himax_master_write(hx_i2c_client_point, buf, 2, DEFAULT_RETRY_CNT);
		if (ret < 0)
...
int gt1x_init_node(void)
{
	...
	proc_entry = proc_create(GESTURE_NODE, 0666, NULL, &ht1x_fops);
	if (proc_entry == NULL) {
		//FTP_ERROR("Couldn't create proc entry[GESTURE_NODE]!");
		return -1;
	} else {
		I("Create proc entry[GESTURE_NODE] success!");
	}
	return 0;
}
int GestureTranslate(int gesture_id)
{
	...
	case EV_GESTURE_PWR:
		//KEY_EVENT = KEY_POWER;
		h_gtp_gesture=GESTURE_DC;
	break;
	case EV_GESTURE_01:
		//KEY_EVENT = KEY_CUST_01;
		h_gtp_gesture=GESTURE_up;
	break;
	case EV_GESTURE_02:
		//KEY_EVENT = KEY_CUST_02;
		h_gtp_gesture=GESTURE_down;
	break;
	case EV_GESTURE_03:
		//KEY_EVENT = KEY_CUST_03;
		h_gtp_gesture=GESTURE_LF;
	break;
	case EV_GESTURE_04:
		//KEY_EVENT = KEY_CUST_04;
		h_gtp_gesture=GESTURE_RT;
	break;
	case EV_GESTURE_05:
		//KEY_EVENT = KEY_CUST_05;
		h_gtp_gesture=GESTURE_c;
	break;
	case EV_GESTURE_06:
		//KEY_EVENT = KEY_CUST_06;
		h_gtp_gesture=GESTURE_z;
	break;
	case EV_GESTURE_07:
		//KEY_EVENT = KEY_CUST_07;
		h_gtp_gesture=GESTURE_m;
	break;
	case EV_GESTURE_08:
		//KEY_EVENT = KEY_CUST_08;
		h_gtp_gesture=GESTURE_o;
	break;
	case EV_GESTURE_09:
		//KEY_EVENT = KEY_CUST_09;
		h_gtp_gesture=GESTURE_s;
	break;
	case EV_GESTURE_10:
		//KEY_EVENT = KEY_CUST_10;
		h_gtp_gesture=GESTURE_v;
	break;
	case EV_GESTURE_11:
		//KEY_EVENT = KEY_CUST_11;
		h_gtp_gesture=GESTURE_w;
	break;
	case EV_GESTURE_12:
		//KEY_EVENT = KEY_CUST_12;
		h_gtp_gesture=GESTURE_e;
	break;
	/*case EV_GESTURE_13:
		KEY_EVENT = KEY_CUST_13;
	break;
	...
	if(get_psval){
		psstatus = get_psval();
		I("han_test_ps psstatus = %d\n",psstatus);
		if(psstatus == 1){
		    //ps is closed,can't report key
		    I("Can't wakeup by tp , ps value cover by something \n");
		}
	}
	else {
		I("get_psval() is not open!\n");
	}
		
	gesture_data.data[0] = h_gtp_gesture;
    gesture_data.data[1] = 0;
    gesture_data.data[2] = 0;
    gesture_data.data[3] = 0;

	if(gesture_id && psstatus == 0) {
		I(" %s SMART WAKEUP KEY event %x press\n",__func__,KEY_POWER);
		input_report_key(private_ts->input_dev, KEY_F13, 1);
		input_sync(private_ts->input_dev);
		//msleep(100);
		I(" %s SMART WAKEUP KEY event %x release\n",__func__,KEY_POWER);
		input_report_key(private_ts->input_dev, KEY_F13, 0);
		input_sync(private_ts->input_dev);
		FAKE_POWER_KEY_SEND=true;
...
static int himax_touch_proc_init(void)
{
	...
	himax_proc_GESTURE_file = proc_create(HIMAX_PROC_GESTURE_FILE, (S_IWUSR|S_IRUGO|S_IWUGO),
		himax_touch_proc_dir, &ht1x_fops);
...
static int himax_parse_dt(struct himax_ts_data *ts,struct himax_i2c_platform_data *pdata)
{
	...
	himax_tpd_rst_gpio_number= 0;//of_get_named_gpio(dt, "rst-gpio", 0);
	himax_tpd_int_gpio_number = 1;//of_get_named_gpio(dt, "int-gpio", 0);
...
#ifdef MTK_KERNEL_310// Allocate the MTK's DMA memory
...
static void himax852xes_suspend(struct device *dev)
{
	...
	cancel_delayed_work_sync(&ts->himax_chip_monitor);
		...
		ts->pre_finger_mask = 0;
		FAKE_POWER_KEY_SEND=false;
		ret_doze=Htp_enter_doze(ts->client);
		if(ret_doze==0){
			I("enter doze success");
			return ;
		}
		/*buf[0] = 0x8F;
		buf[1] = 0x20;
...
#endif
	//if(gesture_enabled == 0){//Sense On
	
	//}
#ifdef HX_SMART_WAKEUP
		 Htp_leave_doze(ts->client);
		/*//Sense Off
		i2c_himax_write_command(ts->client, HX_CMD_TSSOFF, DEFAULT_RETRY_CNT);
		msleep(40);
		//Sleep in
...
#endif
    .attrs = {
                .attr = himax852xes_attrs,
                .num  = ARRAY_SIZE(himax852xes_attrs),
        },
};
#endif
#ifdef QCT
...
{
	printk("Himax 852xES touch panel driver init\n");
