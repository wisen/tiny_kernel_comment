...
static void parse_header(struct image_header *header, const unsigned char *fw_image)
{
	...
	memcpy(header->product_id, &fw_image[PRODUCT_ID_OFFSET], SYNAPTICS_RMI4_PRODUCT_ID_LENGTH);
...
static int fwu_check_version(void)
{
	...
	/* device config id */
	retval = fwu->fn_ptr->read(fwu->rmi4_data,
				   fwu->f34_fd.ctrl_base_addr, config_id, sizeof(config_id));
	if (retval < 0) {
		dev_err(&i2c_client->dev, "Failed to read config ID (code %d).\n", retval);
		return retval;
	}
	...
	if (config_id[0] == fwu->config_data[0] &&
	    ...
	    config_id[2] == fwu->config_data[2] && config_id[3] == fwu->config_data[3]) {
...
static int fwu_read_f01_device_status(struct f01_device_status *status)
{
	...
	retval = fwu->fn_ptr->read(fwu->rmi4_data,
				   fwu->f01_fd.data_base_addr, status->data, sizeof(status->data));
	if (retval < 0) {
...
static int fwu_read_f34_queries(void)
{
	...
	if (retval < 0) {
		dev_err(&i2c_client->dev, "%s: Failed to read flash properties\n", __func__);
		return retval;
	}
	...
	dev_info(&i2c_client->dev, "%s perm:%d, bl%d, display:%d\n",
		 ...
		 fwu->flash_properties.has_bl_config, fwu->flash_properties.has_display_config);
	...
	retval = fwu->fn_ptr->read(fwu->rmi4_data,
				   fwu->f34_fd.query_base_addr + BLOCK_SIZE_OFFSET, buf, 2);
	if (retval < 0) {
		dev_err(&i2c_client->dev, "%s: Failed to read block size info\n", __func__);
		return retval;
	}
	...
	retval = fwu->fn_ptr->read(fwu->rmi4_data,
				   fwu->f34_fd.query_base_addr + FW_BLOCK_COUNT_OFFSET, buf, count);
	if (retval < 0) {
		dev_err(&i2c_client->dev, "%s: Failed to read block count info\n", __func__);
		return retval;
	}
...
static int fwu_read_f34_flash_status(void)
{
	...
	retval = fwu->fn_ptr->read(fwu->rmi4_data,
				   ...
				   fwu->flash_control.data, sizeof(fwu->flash_control.data));
	if (retval < 0) {
...
static int fwu_reset_device(void)
{
	...
	retval = fwu->fn_ptr->write(fwu->rmi4_data,
				    fwu->f01_fd.cmd_base_addr, &reset, sizeof(reset));
	if (retval < 0) {
		dev_err(&fwu->rmi4_data->i2c_client->dev,
...
static int fwu_write_f34_command(unsigned char cmd)
{
	...
	retval = fwu->fn_ptr->write(fwu->rmi4_data, fwu->addr_f34_flash_control, &cmd, sizeof(cmd));
	if (retval < 0) {
...
static int fwu_scan_pdt(void)
{
	...
	for (addr = PDT_START; addr > PDT_END; addr -= PDT_ENTRY_SIZE) {
		retval = fwu->fn_ptr->read(fwu->rmi4_data,
					   addr, (unsigned char *)&rmi_fd, sizeof(rmi_fd));
		if (retval < 0)
			return retval;
		...
		if (rmi_fd.fn_number) {
			...
			case SYNAPTICS_RMI4_F01:
				f01found = true;
				fwu->f01_fd = rmi_fd;
				fwu->addr_f01_interrupt_register = fwu->f01_fd.data_base_addr + 1;
				break;
			    ...
			    unsigned char command)
{
	...
	for (block_num = 0; block_num < block_cnt; block_num++) {
		...
		if (block_num % progress == 0) {
			TPD_DMESG("%s: update %s %3d / %3d\n",
				  ...
				  "config" : "firmware", block_num, block_cnt);
		}
#endif
	...
	dev_info(&fwu->rmi4_data->i2c_client->dev,
		 "%s: update %s %3d / %3d\n",
		 __func__,
		 command == CMD_WRITE_CONFIG_BLOCK ? "config" : "firmware", block_cnt, block_cnt);
#endif
	return 0;
}
...
static int fwu_enter_flash_prog(void)
{
	...
	if (!f01_device_status.flash_prog) {
		dev_err(&fwu->rmi4_data->i2c_client->dev, "%s: Not in flash prog mode\n", __func__);
		return -EINVAL;
	}
	...
	retval = fwu->fn_ptr->read(fwu->rmi4_data,
				   ...
				   f01_device_control.data, sizeof(f01_device_control.data));
	if (retval < 0) {
...
static int fwu_start_reflash(void)
{
	...
	if (fwu->ext_data_source)
		...
		if (retval != 0) {
			TPD_DMESG("%s: Firmware image %s not available\n", __func__, FW_IMAGE_NAME);
			retval = -EINVAL;
			goto exit;
		}

		TPD_DMESG("%s: Firmware image size = %d\n", __func__, fw_entry->size);
	...
	if (header.config_size)
		fwu->config_data = fw_image + FW_IMAGE_OFFSET + header.image_size;
...
static int fwu_do_write_config(void)
{
	...
	TPD_DMESG("%s: Erase command written\n", __func__);
...
static int fwu_start_write_config(void)
{
	...
	if (retval < 0)
		dev_err(&fwu->rmi4_data->i2c_client->dev, "%s: Failed to write config\n", __func__);
...
static int fwu_do_read_config(void)
{
	...
	for (block_num = 0; block_num < block_count; block_num++) {
		...
		retval = fwu->fn_ptr->read(fwu->rmi4_data,
					   fwu->f34_fd.data_base_addr + BLOCK_DATA_OFFSET,
					   &fwu->read_config_buf[index], fwu->block_size);
		if (retval < 0) {
			dev_err(&fwu->rmi4_data->i2c_client->dev,
...
static ssize_t fwu_sysfs_store_image(struct file *data_file,
				     ...
				     char *buf, loff_t pos, size_t count)
{
	memcpy((void *)(&fwu->ext_data_source[fwu->data_pos]), (const void *)buf, count);
					  ...
					  size_t count)
{
	...
	if (retval < 0) {
		dev_err(&rmi4_data->i2c_client->dev, "%s: Failed to do reflash\n", __func__);
		goto exit;
	}
					    ...
					    size_t count)
{
	...
	if (retval < 0) {
		dev_err(&rmi4_data->i2c_client->dev, "%s: Failed to write config\n", __func__);
		goto exit;
	}
					   ...
					   size_t count)
{
	...
	if (retval < 0) {
		dev_err(&rmi4_data->i2c_client->dev, "%s: Failed to read config\n", __func__);
		return retval;
	}
...
static void synaptics_rmi4_fwu_attn(struct i2c_client *client, unsigned char intr_mask)
{
 ...
 */
#if 0
static int synaptics_rmi4_set_page(struct synaptics_rmi4_data *rmi4_data, unsigned int address)
{
	int retval = 0;
	unsigned char retry;
					...
					struct synaptics_rmi4_fn *fhandler)
{
	...
	f1a->button_map = kcalloc(f1a->button_count, sizeof(*(f1a->button_map)), GFP_KERNEL);
	if (!f1a->button_map) {
		/*dev_err(&rmi4_data->i2c_client->dev,
...
static int synaptics_rmi4_fwu_init(struct i2c_client *client)
{
	...
	if (!fwu) {
		/*dev_err(&client->dev, "%s: Failed to alloc mem for fwu\n", __func__);*/
		goto exit;
	}
	...
	if (!fwu->fn_ptr) {
		/*dev_err(&client->dev, "%s: Failed to alloc mem for fn_ptr\n", __func__);*/
		retval = -ENOMEM;
		goto exit_free_rmi4;
	}
	...
	retval = fwu->fn_ptr->read(fwu->rmi4_data,
				   PDT_PROPS, pdt_props.data, sizeof(pdt_props.data));
	if (retval < 0) {
...
*/
	...
	TPD_DMESG
	    ("query_base_addr=0x%x, data_base_addr=0x%x, addr_f34_flash_control=0x%x, bootloader_id=%s\n",
	     fwu->f34_fd.query_base_addr, fwu->f34_fd.data_base_addr, fwu->addr_f34_flash_control,
	     fwu->bootloader_id);
...
/* fwu->config_area = UI_CONFIG_AREA; */
	...
	if (!properties_kobj_fwupdate) {
		dev_err(&client->dev, "%s: Failed to create sysfs directory\n", __func__);
		goto err_sysfs_dir;
	}
	retval = sysfs_create_bin_file(properties_kobj_fwupdate, &dev_attr_data);
	if (retval < 0) {
		dev_err(&client->dev, "%s: Failed to create sysfs bin file\n", __func__);
		goto exit_free_mem;
	}
...
static void synaptics_rmi4_fwu_remove(struct i2c_client *client)
{
	...
	for (attr_count = 0; attr_count < ARRAY_SIZE(attrs); attr_count++)
		sysfs_remove_file(&fwu->rmi4_data->input_dev->dev.kobj, &attrs[attr_count].attr);
