...
#define	GF_KEY_MENU		KEY_APPSELECT
...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case GF_IOC_SENDKEY:
		...
		if(gf_key.key==KEY_MENU)
			gf_key.key=GF_KEY_MENU;
...
extern void app_get_fingerprint_name(char *name);  // muduo.qin@20161025, for rlkdevinfo
static int gf_probe(struct spi_device *spi)
{
	struct gf_dev *gf_dev = &gf;
	int status = -EINVAL;
	...
	app_get_fingerprint_name("GX556"); // muduo.qin@20161025, for rlkdevinfo
...
static ssize_t gf_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
    ...
    if ((count > len)||(count == 0)) {
        gf_print("Max size for write buffer is %d. wanted length is %u\n", len, (unsigned int)count);
        FUNC_EXIT();
        return -EMSGSIZE;
    }
