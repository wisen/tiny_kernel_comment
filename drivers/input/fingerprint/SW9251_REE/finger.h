...
#define  sw_dbg(fmt, args...)     do{if(sunwave_debug_level){  printk("sunwave:  %d " fmt " \n",__LINE__, ##args);}}while(0)
#define  sw_info(fmt, args...)    do{printk("sunwave:  %d " fmt " \n",__LINE__, ##args);}while(0)
#define  sw_err(fmt, args...)     do{printk(KERN_ERR"sunwave: error:  %d " fmt " \n",__LINE__, ##args);}while(0)
