...
#define SUNWAVE_VERSION "V0.8.20160909"
...
extern struct mt_chip_conf sunwave_chip_config;
...
static int __devinit sunwave_dev_probe(struct spi_device* spi)
{
    ...
    sunwave_dev->spi = spi;
	sunwave_dev->spi->controller_data = (void*) &sunwave_chip_config;
	spi_setup(spi);
	...
	//power on
	if (sunwave_dev->finger && sunwave_dev->finger->power) {
        status = sunwave_dev->finger->power(&sunwave_dev->spi);
    	}
	...
	app_get_fingerprint_name("SW9362T");
...
static int __init sunwave_dev_init(void)
{
	...
	if(!sunwave_driver_status){
		printk("[%s] compatible driver error\n",__func__);
		driver_unregister(&sunwave_dev_spi_driver.driver);
	}
    return status;
}
late_initcall(sunwave_dev_init);
