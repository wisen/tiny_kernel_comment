...
/*  fangmengyao add for compatible batteries with GPIO START 20161019  */
#include <linux/gpio.h>
/*  fangmengyao add for compatible batteries with GPIO END  20161019 */
...
/*  fangmengyao add for compatible batteries with GPIO START 20161019  */
struct platform_device *bat_meter_Pltfmdev;
struct platform_device *batmeterPltFmDev;

struct platform_device * get_bat_meter_platformdev(void){
	return bat_meter_Pltfmdev;
}
/*  fangmengyao add for compatible batteries with GPIO END 20161019  */

#ifdef MTK_MULTI_BAT_PROFILE_SUPPORT
#if defined(CONFIG_E31)
struct pinctrl *bat_id_pinctrl;
struct pinctrl_state *bat_pin_default;
struct pinctrl_state *bat_id;
#endif
...
void fgauge_get_profile_id(void)
{
	...
	int id_volt = 0;
	int ret;			
	batmeterPltFmDev = get_bat_meter_platformdev();	
	/* gpio setting */	
	bat_id_pinctrl = devm_pinctrl_get(&batmeterPltFmDev->dev);	/* get pinctrl */
	if (IS_ERR(bat_id_pinctrl)) {		
		ret = PTR_ERR(bat_id_pinctrl);		
		printk("Cannot find battery_meter pinctrl!\n");	
	}	
	bat_pin_default = pinctrl_lookup_state(bat_id_pinctrl, "bat_pin_default");	
	if (IS_ERR(bat_pin_default)) {		
		ret = PTR_ERR(bat_pin_default);		
		printk("Cannot find battery_meter pinctrl bat_pin_default!\n");	
	}	
	bat_id = pinctrl_lookup_state(bat_id_pinctrl, "bat_id");	
	if (IS_ERR(bat_id)) {		
		ret = PTR_ERR(bat_id);		
		printk("Cannot find battery_meter pinctrl bat_id!\n");	
	}	
	pinctrl_select_state(bat_id_pinctrl, bat_id);	
	id_volt = gpio_get_value(GPIO80);		/*get id pin state */
	printk("fmy id_volt = %d \n",id_volt);			
	if(id_volt == 0){		
		g_fg_battery_id = 0;	
	}else if(id_volt == 1){		
		g_fg_battery_id = 1;	
