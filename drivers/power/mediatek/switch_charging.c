...
#if defined(CHANGE_CURRENT_BY_TEMP)
#include <linux/thermal.h>
#include <mt-plat/mtk_thermal_monitor.h>
#if defined (CONFIG_E31)
static long E31_PCB_temperature = 0;
#else
static long PCB_temperature = 0;
#endif
#endif
...
#if defined(CHANGE_CURRENT_BY_TEMP)
CHR_CURRENT_ENUM g_temp_CC_value_change = CHARGE_CURRENT_1225_00_MA;
CHR_CURRENT_ENUM g_temp_input_CC_value_change = CHARGE_CURRENT_1500_00_MA;
#endif
...
#if defined(CHANGE_CURRENT_BY_TEMP)
extern struct thermal_zone_device *g_pa_zone;
extern unsigned long PCB_temperature; 
#endif
...
static BATTERY_VOLTAGE_ENUM select_jeita_cv(void)
{
	...
	} else if (g_temp_status == TEMP_POS_15_TO_POS_45) {
		if (batt_cust_data.high_battery_voltage_support)
			cv_voltage = BATTERY_VOLT_04_400000_V;
		else
			cv_voltage = JEITA_TEMP_POS_15_TO_POS_45_CV_VOLTAGE;
	} else if (g_temp_status == TEMP_POS_0_TO_POS_15) {
		cv_voltage = JEITA_TEMP_POS_0_TO_POS_15_CV_VOLTAGE;
	} else if (g_temp_status == TEMP_BELOW_0) {
		cv_voltage = JEITA_TEMP_BELOW_0_CV_VOLTAGE;
	} else {
...
PMU_STATUS do_jeita_state_machine(void)
{
	...
	if (BMT_status.temperature >= TEMP_POS_60_THRESHOLD) {
		...
		if((g_temp_status == TEMP_POS_15_TO_POS_45) && (BMT_status.temperature < TEMP_POS_45_THRES_PLUS_X_DEGREE)){ 
			battery_log(BAT_LOG_CRTI, "[BATTERY] Battery Temperature not recovery to normal temperature charging mode yet!!\n\r"); 
		}else {
			battery_log(BAT_LOG_CRTI,
	...
	} else if (BMT_status.temperature >= TEMP_POS_15_THRESHOLD) {
		if (((g_temp_status == TEMP_POS_45_TO_POS_60)
			...
			&& (BMT_status.temperature <= TEMP_POS_15_THRES_PLUS_X_DEGREE))) {
			battery_log(BAT_LOG_CRTI,
			...
			battery_log(BAT_LOG_CRTI,
				    ...
				    TEMP_POS_15_THRESHOLD, TEMP_POS_45_THRESHOLD);
			g_temp_status = TEMP_POS_15_TO_POS_45;
		}
	} else if (BMT_status.temperature > TEMP_POS_0_THRESHOLD) {
		if((g_temp_status == TEMP_POS_15_TO_POS_45) && (BMT_status.temperature >TEMP_POS_15_THRES_MINUS_X_DEGREE)){
			battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] Battery Temperature not recovery to normal temperature charging mode yet!!\n\r");
		}	    
		if((g_temp_status == TEMP_BELOW_0) && (BMT_status.temperature < TEMP_POS_0_THRES_PLUS_X_DEGREE)){
			...
			battery_log(BAT_LOG_CRTI,
				    "[BATTERY] Battery Temperature between %d and %d !!\n\r",
				    TEMP_POS_0_THRESHOLD, TEMP_POS_15_THRESHOLD);
	...
	if (g_temp_status != TEMP_POS_15_TO_POS_45) {
...
static void set_jeita_charging_current(void)
{
	...
	if (BMT_status.charger_type == STANDARD_HOST)
		return;
#endif
	if(g_temp_status == TEMP_BELOW_0) 
	{		
		g_temp_CC_value = CHARGE_CURRENT_0_00_MA;	
		g_temp_input_CC_value = CHARGE_CURRENT_0_00_MA;
		battery_log(BAT_LOG_CRTI, "[BATTERY] TEMP_BELOW_NEG_0 JEITA set charging current : %d\r\n", g_temp_CC_value);
	}else if(g_temp_status == TEMP_POS_0_TO_POS_15){
		...
		battery_log(BAT_LOG_CRTI, "[BATTERY] TEMP_POS_0_TO_POS_15 JEITA set charging current : %d\r\n", g_temp_CC_value);
	}else if(g_temp_status == TEMP_POS_15_TO_POS_45){
		if ( BMT_status.charger_type  == STANDARD_CHARGER )
		{ 
		...
		battery_log(BAT_LOG_CRTI,"[BATTERY] TEMP_POS_15_TO_POS_45 JEITA set charging current : %d\r\n", g_temp_CC_value);
	}else if(g_temp_status == TEMP_POS_45_TO_POS_60){
		if ( BMT_status.charger_type  == STANDARD_CHARGER )
		{ 
			g_temp_CC_value = CHARGE_CURRENT_1225_00_MA;
		...
		battery_log(BAT_LOG_CRTI, "[BATTERY] TEMP_POS_45_TO_POS_60 JEITA set charging current : %d\r\n", g_temp_CC_value);
	}else if(g_temp_status == TEMP_ABOVE_POS_60){
		g_temp_CC_value = CHARGE_CURRENT_0_00_MA;
		g_temp_input_CC_value = CHARGE_CURRENT_0_00_MA;
		battery_log(BAT_LOG_CRTI, "[BATTERY] JEITA set charging current : %d\r\n", g_temp_CC_value);
	}else{
		battery_log(BAT_LOG_CRTI,"[BATTERY] TEMP_POS_default JEITA set charging current : %d\r\n", g_temp_CC_value);
	}	
}
#else
static BATTERY_VOLTAGE_ENUM select_jeita_cv(void)
...
#if defined (CONFIG_E31)
void change_current_by_temp(void)
{
	if(g_platform_boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT || g_platform_boot_mode == LOW_POWER_OFF_CHARGING_BOOT){
				g_temp_input_CC_value_change = CHARGE_CURRENT_1500_00_MA;
				g_temp_CC_value_change = CHARGE_CURRENT_1225_00_MA;
	}else{
		if(E31_PCB_temperature >= 42000){
			g_temp_input_CC_value_change = CHARGE_CURRENT_1000_00_MA;
			g_temp_CC_value_change = CHARGE_CURRENT_1000_00_MA;
			if(E31_PCB_temperature >= 43000){	
				g_temp_input_CC_value_change = CHARGE_CURRENT_500_00_MA;
				g_temp_CC_value_change = CHARGE_CURRENT_500_00_MA;
			}
		}else{
			g_temp_input_CC_value_change = CHARGE_CURRENT_1500_00_MA;
			g_temp_CC_value_change = CHARGE_CURRENT_1225_00_MA;
...
void change_current_by_temp(void)
{
	if(PCB_temperature > 43000){
		g_temp_input_CC_value_change = CHARGE_CURRENT_1000_00_MA;
		g_temp_CC_value_change = CHARGE_CURRENT_1000_00_MA;
		if(PCB_temperature > 45000){
			g_temp_input_CC_value_change = CHARGE_CURRENT_800_00_MA;
			g_temp_CC_value_change = CHARGE_CURRENT_800_00_MA;
		}
	}else{
		g_temp_input_CC_value_change = CHARGE_CURRENT_1950_00_MA;
		g_temp_CC_value_change = CHARGE_CURRENT_1950_00_MA;
	}
}
#endif
...
#if defined(CHANGE_CURRENT_BY_TEMP)
#if defined (CONFIG_E31)
		E31_PCB_temperature = mtk_thermal_get_temp(MTK_THERMAL_SENSOR_AP);
		printk("E31 PCB  Temperature = %ld\n", E31_PCB_temperature);
#else
		thermal_zone_get_temp(g_pa_zone,&PCB_temperature);
		printk("pcb temp = %ld\n", PCB_temperature);
#endif
		change_current_by_temp();
		g_temp_input_CC_value = MIN(g_temp_input_CC_value,g_temp_input_CC_value_change);
		g_temp_CC_value = MIN(g_temp_CC_value,g_temp_CC_value_change);
		 battery_log(BAT_LOG_CRTI,"[BATTERY] After change CC mode charging current = %d, inpt current = %d\n",g_temp_CC_value, g_temp_input_CC_value);
#endif
	}
...
#if defined(CONFIG_MTK_JEITA_STANDARD_SUPPORT)
#if defined(CONFIG_E31)
	if ((g_temp_status == TEMP_ABOVE_POS_60) || (g_temp_status == TEMP_BELOW_0))
