...
int first_init_update = 0; /*fangmengyao add 20161019*/
/* ////////////////////////////////////////////////////////////////////////////// */
...
#endif
#endif
	if(BMT_status.charger_exist == KAL_TRUE){	/*fangmengyao add start 20161116*/
		power_supply_changed(bat_psy);
	}else{										/*fangmengyao add end 20161116*/
		if (update_cnt == 6) {
			...
			power_supply_changed(bat_psy);
			cable_in_uevent = 0;
		}else if(first_init_update == 1){	/*fangmengyao add start 20161019*/
		/*add for KPOC showing percentage picture */
			power_supply_changed(bat_psy);
			first_init_update = 0;	/*fangmengyao add end 20161019*/
		} else {
			/* No update */
			update_cnt++;
		}
...
#if defined (CONFIG_E31)	// fangmengyao - 20161028 - add for kernel charger type detection START
	int charger_type_cnt;
#endif	// fangmengyao - 20161028 - add for kernel charger type detection END
...
#if defined (CONFIG_E31)		// fangmengyao - 20161028 - add for kernel charger type detection START
	if(BMT_status.charger_type != STANDARD_CHARGER){
		battery_log(BAT_LOG_CRTI, "-fmy- line:%d[%s] [Cable_Not_equal_STANDARD_CHARGER] BMT_status.charger_type = %d \n",__LINE__, __FUNCTION__, BMT_status.charger_type);
		for(charger_type_cnt = 0; charger_type_cnt <2; charger_type_cnt++ ){
			battery_charging_control(CHARGING_CMD_GET_CHARGER_TYPE, &CHR_Type_num);
			BMT_status.charger_type = CHR_Type_num;
			battery_log(BAT_LOG_CRTI, "-fmy- line:%d[%s] [Cable_Not_equal_STANDARD_CHARGER] BMT_status.charger_type = %d , charger_type_cnt = %d\n",__LINE__, __FUNCTION__, BMT_status.charger_type, charger_type_cnt);
			if(BMT_status.charger_type == STANDARD_CHARGER){
				break;
			}			
		}		
...
{
#if defined(CONFIG_POWER_EXT) || 0/*defined()*/
...
static int battery_probe(struct platform_device *dev)
{
	...
	if (g_vcdt_irq_delay_flag == KAL_TRUE)
		do_chrdet_int_task();
#endif
	first_init_update = 1;	/*fangmengyao add 20161019*/
	return 0;
