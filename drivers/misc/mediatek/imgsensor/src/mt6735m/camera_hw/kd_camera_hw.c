...
//struct pinctrl_state *cam0_ldo0_h = NULL;
struct pinctrl_state *cam0_ldo0_l = NULL;
struct pinctrl_state *cam1_ldo0_h = NULL;
struct pinctrl_state *cam1_ldo0_l = NULL;
int mtkcam_gpio_init(struct platform_device *pdev)
{
	...
	/*externel LDO enable */
	/*
	cam0_ldo0_h = pinctrl_lookup_state(camctrl, "cam0_ldo0_1");
	if (IS_ERR(cam0_ldo0_h)) {
		ret = PTR_ERR(cam0_ldo0_h);
		pr_debug("%s : pinctrl err, cam0_ldo0_h\n", __func__);
	}


	cam0_ldo0_l = pinctrl_lookup_state(camctrl, "cam0_ldo0_0");
	if (IS_ERR(cam0_ldo0_l)) {
		ret = PTR_ERR(cam0_ldo0_l);
		pr_debug("%s : pinctrl err, cam0_ldo0_l\n", __func__);
	}
	*/
//front
	cam1_ldo0_h = pinctrl_lookup_state(camctrl, "cam1_ldo0_1");
	if (IS_ERR(cam1_ldo0_h)) {
		ret = PTR_ERR(cam1_ldo0_h);
		pr_debug("%s : pinctrl err, cam1_ldo0_h\n", __func__);
	}


	cam1_ldo0_l = pinctrl_lookup_state(camctrl, "cam1_ldo0_0");
	if (IS_ERR(cam1_ldo0_l)) {
		ret = PTR_ERR(cam1_ldo0_l);
		pr_debug("%s : pinctrl err, cam1_ldo0_l\n", __func__);
	}
...
int mtkcam_gpio_set(int PinIdx, int PwrType, int Val)
{
	...
	case CAMLDO_0:
		if (Val == 0)
			pinctrl_select_state(camctrl, cam0_ldo0_l);
		else
			pinctrl_select_state(camctrl, cam0_ldo0_h);
		break;
		*/
	case CAMLDO_1:
		if (Val == 0)
			pinctrl_select_state(camctrl, cam1_ldo0_l);
		else
			pinctrl_select_state(camctrl, cam1_ldo0_h);
		break;
	default:
...
void checkPowerBeforClose(char *mode_name)
{
	...
	for (i = 0; i < cntVRF18_1; i++)
		_hwPowerDown(VRF18_1);
	cntVCAMD = 0;
	cntVCAMA = 0;
	cntVCAMIO = 0;
	cntVCAMAF = 0;
		       ...
		       char *mode_name)
{
	...
	if (On) 
	{
		...
		PK_DBG("[PowerON]pinSetIdx:%d, currSensorName: %s\n", pinSetIdx, currSensorName);
              if(currSensorName && ((0 == strcmp(currSensorName,SENSOR_DRVNAME_S5K3H7YX_MIPI_RAW))||(0 == strcmp(currSensorName,SENSOR_DRVNAME_S5K3L8_MIPI_RAW))))
		{
			if (GPIO_CAMERA_INVALID != pinSet[1-pinSetIdx][IDX_PS_CMPDN]) //set  sub camera pdwn gpio to low when main camera power on 
			{
				mtkcam_gpio_set((1-pinSetIdx), CAMPDN,pinSet[1-pinSetIdx][IDX_PS_CMPDN + IDX_PS_OFF]);
			}
			
			if (GPIO_CAMERA_INVALID != pinSet[1-pinSetIdx][IDX_PS_CMRST])  //set sub camera reset to low when  main camera power on
			{
			...
			if (TRUE != _hwPowerOnCnt(VCAMIO, VOL_1800, mode_name))
			{
				...
				goto _kdCISModulePowerOn_exit_;
	            	...
	            	mdelay(1);
	        //  mtkcam_gpio_set(pinSetIdx, CAMLDO_0, 1);
		...
		else if(currSensorName && (0 == strcmp(currSensorName,SENSOR_DRVNAME_OV5695_MIPI_RAW)))
		{
			if (GPIO_CAMERA_INVALID != pinSet[1-pinSetIdx][IDX_PS_CMPDN])//set main camera pdwn gpio to low when  sub camera power
			{
				mtkcam_gpio_set((1-pinSetIdx), CAMPDN,pinSet[1-pinSetIdx][IDX_PS_CMPDN + IDX_PS_OFF]);
			}


			if (GPIO_CAMERA_INVALID != pinSet[1-pinSetIdx][IDX_PS_CMRST])//set  main camera reset gpio to low when sub camera power on
			{
				mtkcam_gpio_set((1-pinSetIdx), CAMRST,pinSet[1-pinSetIdx][IDX_PS_CMRST + IDX_PS_OFF]);
			}
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			if (TRUE != _hwPowerOnCnt(VCAMIO , VOL_1800, mode_name))
			{
	           ...
	           // 	mdelay(1);
			//AVDD
			mtkcam_gpio_set(pinSetIdx, CAMLDO_1, 1); //LDO 
			...
			if (TRUE != _hwPowerOnCnt(VCAMA, VOL_2800, mode_name))
			{
	         ...
	         //   	mdelay(1);
			//DVDD
			if (TRUE != _hwPowerOnCnt(VRF18_1, VOL_1200, mode_name)) 
			{
	 ...
	 {		/* power OFF */
		 ...
		 if(currSensorName && ((0 == strcmp(currSensorName,SENSOR_DRVNAME_S5K3H7YX_MIPI_RAW))||(0 == strcmp(currSensorName,SENSOR_DRVNAME_S5K3L8_MIPI_RAW))))
		{
			...
			{
				PK_DBG("[CAMERA SENSOR] Fail to OFF AF power (VCAM_AF), power id = %d,pinSetIdx:%d\n",VCAMAF,pinSetIdx);
				/* return -EIO; */
				goto _kdCISModulePowerOn_exit_;
			}
			if (TRUE != _hwPowerDownCnt(VCAMD, mode_name)) 
			{
				PK_DBG("[CAMERA SENSOR] Fail to OFF core power (VCAM_D), power id = %d,pinSetIdx:%d\n",VCAMD,pinSetIdx);
				goto _kdCISModulePowerOn_exit_;
			}
				/* VCAM_A */
			//	mtkcam_gpio_set(pinSetIdx, CAMLDO_0, 0);//LDO
			if (TRUE != _hwPowerDownCnt(VCAMA, mode_name)) 
			{
				PK_DBG("[CAMERA SENSOR] Fail to OFF analog power (VCAM_A), power id= (%d),pinSetIdx :%d\n",VCAMA,pinSetIdx);
				goto _kdCISModulePowerOn_exit_;
			}
			/* VCAM_IO */
			if (TRUE != _hwPowerDownCnt(VCAMIO, mode_name))
			{
				PK_DBG("[CAMERA SENSOR] Fail to OFF digital power (VCAM_IO), power id = %d, pinSetIdx:%d\n",VCAMIO,pinSetIdx);
				goto _kdCISModulePowerOn_exit_;
			}
		}
		else if(currSensorName && (0 == strcmp(currSensorName,SENSOR_DRVNAME_OV5695_MIPI_RAW)))
		{
			...
			mdelay(1);
			/* VCAM_A */
			if (TRUE != _hwPowerDownCnt(VRF18_1, mode_name)) 
			{
				PK_DBG("[CAMERA SENSOR] Fail to OFF analog power (VRF18_1), power id= (%d),pinSetIdx:%d\n",VRF18_1,pinSetIdx);
				goto _kdCISModulePowerOn_exit_;
			}

			mtkcam_gpio_set(pinSetIdx, CAMLDO_1, 0);
			if (TRUE != _hwPowerDownCnt(VCAMA, mode_name))
			{
				PK_DBG("[CAMERA SENSOR] Fail to OFF core power (VCAMA), power id = %d,pinSetIdx:%d\n",VCAMA,pinSetIdx);
				goto _kdCISModulePowerOn_exit_;
			}
			/* VCAM_IO */
			if (TRUE != _hwPowerDownCnt(VCAMIO, mode_name)) 
