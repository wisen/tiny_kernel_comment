...
OV8855_VERSION ov8855version = OV8855R1A;

static DEFINE_SPINLOCK(imgsensor_drv_lock);

static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = OV8855_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
			...
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			if (*sensor_id == imgsensor_info.sensor_id) {
				if((read_cmos_sensor(0x302A)) == 0XB0){
 				ov8855version = OV8855R1A;
				LOG_INF("i2c write id: 0x%x, sensor id: 0x%x, ov8855version = %d(0=r2a,1=r1a)\n", imgsensor.i2c_write_id,*sensor_id,ov8855version);	
				app_get_front_sensor_name("ov8855_ofilm");
				return ERROR_NONE;
				}
				//else if((read_cmos_sensor(0x302A)) == 0XB1){
 				//ov8855version = OV8855R2A;
				//LOG_INF("i2c write id: 0x%x, sensor id: 0x%x, ov8855version = %d(0=r2a,1=r1a)\n", imgsensor.i2c_write_id,*sensor_id,ov8855version);	
				//return ERROR_NONE;
				//}
				//else{
				//LOG_INF("read ov8855 R1A R2A bate fail\n");	  
...
UINT32 OV8855_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
