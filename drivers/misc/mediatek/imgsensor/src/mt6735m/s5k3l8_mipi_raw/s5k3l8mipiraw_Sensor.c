...
#define SHUTTER_1MS      97 
#define SHUTTER_2MS      195
#define SHUTTER_10MS    975
#define SHUTTER_33MS    3217
#define SHUTTER_50MS    4875
#define SHUTTER_66MS    6435


#define SHUTTER_100MS   9751
#define SHUTTER_125MS   12189
#define SHUTTER_200MS  19502
#define SHUTTER_500MS   48756
#define SHUTTER_1S 		97513
#define SHUTTER_5S 		487567
#define SHUTTER_10S 	975134
...
static imgsensor_info_struct imgsensor_info = { 
	...
	.pre = {
		...
		.framelength = 2752,			//record different mode's framelength
	...
	.cap ={
		.pclk = 480000000,				//record different mode's pclk
		.linelength  = 5808,				//record different mode's linelength
		.framelength = 3179,			//record different mode's framelength
		.startx = 0,					//record different mode's startx of grabwindow
		.starty = 0,					//record different mode's starty of grabwindow
	...
	.cap ={
		...
		.linelength  = 5808,				//record different mode's linelength
		.framelength = 3482,			//record different mode's framelength
		.startx = 0,					//record different mode's startx of grabwindow
		.starty = 0,					//record different mode's starty of grabwindow
		.grabwindow_width  = 4208,		//record different mode's width of grabwindow
		.grabwindow_height = 3120,		//record different mode's height of grabwindow
...
#endif
	.normal_video = {
		.pclk = 528000000,				//record different mode's pclk
		.linelength  = 5808,				//record different mode's linelength
	...
	.hs_video = {
		.pclk = 480000000,				//record different mode's pclk
		.linelength  = 5808,				//record different mode's linelength
		.framelength = 688,			//record different mode's framelength
		.startx= 0,					//record different mode's startx of grabwindow
		.starty = 0,					//record different mode's starty of grabwindow
	...
	.slim_video = {
		.pclk = 480000000,				//record different mode's pclk
		.linelength  = 5808,				//record different mode's linelength
		.framelength = 1376,			//record different mode's framelength
		.startx= 0,					//record different mode's startx of grabwindow
		.starty = 0,					//record different mode's starty of grabwindow
  ...
  .custom5 = {
	...
	.i2c_addr_table = {0x20, /*0x20,*/0xff},//record sensor support all write id addr, only supprt 4must end with 0xff
...
static void set_shutter(kal_uint32 shutter)
{
	unsigned long flags;
	kal_uint16 realtime_fps = 0;
	kal_uint32 shutter_choose = shutter;
//	kal_uint32 frame_length = 0;
	...
	switch(shutter_choose)
            {
		case SHUTTER_1MS: //
		case SHUTTER_2MS: // 10ms 
			LOG_INF("xxxx shutter 10ms = %u\n", shutter_choose);
			write_cmos_sensor(0x0342, 0x2D60);//11616
			write_cmos_sensor(0x0340, 0x0D9A);
			write_cmos_sensor(0x0202, 0x01E8);//
            break;
		case SHUTTER_200MS:
			LOG_INF("xxxx-------shutter1_5 \n ");
			write_cmos_sensor(0x0342, 0x16B0);
			write_cmos_sensor(0x0340, 0x4C40);
			write_cmos_sensor(0x0202, 0x4C30);
		break;

		case SHUTTER_500MS:
			LOG_INF("xxxx-------shutter1_2 \n ");
			write_cmos_sensor(0x0342, 0x16B0);
			write_cmos_sensor(0x0340, 0xBE88);
			write_cmos_sensor(0x0202, 0xBE78);

		break;
		
		case SHUTTER_1S:
			LOG_INF("xxxx-------shutter1 \n ");
			write_cmos_sensor(0x0342, 0x3F86);
			write_cmos_sensor(0x0340, 0x4402);
			write_cmos_sensor(0x0202, 0x43FA);
	 ...
	 case SHUTTER_5S:// 2s
	 		LOG_INF("xxxx-------shutter1_2 \n ");
			write_cmos_sensor(0x0342, 0x3F86);
			write_cmos_sensor(0x0340, 0x51A2);
			write_cmos_sensor(0x0202, 0x5192);
			break;
			
	 case SHUTTER_10S: //5s
			LOG_INF("xxxx-------shutter5 \n ");
			write_cmos_sensor(0x0342, 0x3F86);
			write_cmos_sensor(0x0340, 0x87FD);
			write_cmos_sensor(0x0202, 0x87F5);   
  			break;
		default:
				LOG_INF("xxxx-------shutter:not manul shutter value!=%d,shutter_choose=%d\n",shutter,shutter_choose);
			break;
...
static void sensor_init(void)
{
  ...
  LOG_INF("E\n");
	...
	write_cmos_sensor(0x0100, 0x0003);	// mirror + Flip 0x0101, 0x03
}	/*	sensor_init  */
...
static void preview_setting(void)
{
  ...
  write_cmos_sensor(0x0342, 0x16B0);
  #endif
  write_cmos_sensor(0x0340, 0x0ac0);
...
static void hs_video_setting(void)
{
  ...
  write_cmos_sensor(0x0308, 0x0008);
  write_cmos_sensor(0x0342, 0x16B0);
  write_cmos_sensor(0x0340, 0x02b0);
  write_cmos_sensor(0x0202, 0x0200);
  write_cmos_sensor(0x0200, 0x00C6);
  write_cmos_sensor(0x0B04, 0x0101);
  write_cmos_sensor(0x0B08, 0x0000);
...
static void slim_video_setting(void)
{
  ...
  write_cmos_sensor(0x0308, 0x0008);
  write_cmos_sensor(0x0342, 0x16B0);
  write_cmos_sensor(0x0340, 0x0560);
  write_cmos_sensor(0x0202, 0x0200);
  write_cmos_sensor(0x0200, 0x00C6);
  write_cmos_sensor(0x0B04, 0x0101);
  write_cmos_sensor(0x0B08, 0x0000);
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            *sensor_id = return_sensor_id();
			...
			app_get_back_sensor_name("s5k3l8_ofilm");
			return ERROR_NONE;
            }
	   ...
	   case SENSOR_FEATURE_GET_TEMPERATURE_VALUE:
		{
			...
			LOG_INF("{sendCommand} SENSOR_FEATURE_GET_FN_FF_OTP_DATA\n");

			temperatureRead( (int32_t*)feature_return_para_32 );

			LOG_INF("Sending FF temperature: %d\n", *(int32_t*)feature_return_para_32);
			
			*feature_para_len=4;
			break;
		}

	   case SENSOR_FEATURE_GET_FN_FF_OTP_DATA:
		{
			LOG_INF("{sendCommand} SENSOR_FEATURE_GET_FN_FF_OTP_DATA\n");

			ReadOtpAF( (kal_uint8*)feature_return_para_32 );
				
			LOG_INF("Sending FF OTP: %d\n", *feature_return_para_32 );

			*feature_para_len=FDAF_OTP_LEN;
			break;
		}			
#endif
