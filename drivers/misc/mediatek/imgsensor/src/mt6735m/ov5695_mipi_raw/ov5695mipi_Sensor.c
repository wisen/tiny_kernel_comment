...
#include "ov5695_sensor_otp.h"

static DEFINE_SPINLOCK(imgsensor_drv_lock);


static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = OV5695MIPI_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			lens_id = ov5695_read_lensId();
			if (*sensor_id == imgsensor_info.sensor_id) {	
				
				LOG_INF("i2c write id: 0x%x, sensor id: 0x%x,lens_id=%d\n", imgsensor.i2c_write_id,*sensor_id,lens_id);	
				app_get_front_sensor_name("ov5695_ofilm");
				
				return ERROR_NONE;
			}	
			LOG_INF("Read sensor id fail, id: 0x%x,sensor id: 0x%x,lens_id=%d,retry=%d\n", imgsensor.i2c_write_id,*sensor_id,lens_id,retry);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
