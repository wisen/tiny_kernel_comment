...
#define DLMALLOC_EXPORT do {} while (0) /*remove "extern" define for kernel style*/
#endif
...
#define mchunk struct malloc_chunk
#define mchunkptr struct malloc_chunk *
#define sbinptr struct malloc_chunk *  /* The type of bins of chunks */
#define bindex_t unsigned int          /* Described below */
#define binmap_t unsigned int          /* Described below */
#define flag_t unsigned int            /* The type of various bit flag sets */
...
#define tchunk struct malloc_tree_chunk
#define tchunkptr struct malloc_tree_chunk *
#define tbinptr struct malloc_tree_chunk * /* The type of bins of trees */
...
*/
#define msegment struct malloc_segment
#define msegmentptr struct malloc_segment *
...
#define mstate struct malloc_state *
		...
		if (!is_initialized(m)) { /* first-time initialization */
			...
			if (sp != 0 &&
				...
				if (sp != 0 &&
					...
					sp->base = tbase;
					sp->size += tsize;
					ret = prepend_alloc(m, tbase, oldbase, nb);
					goto out;
				} else
...
#ifdef KERNEL_EXTMEM_MSPACE
void *internal_malloc(mspace msp, size_t bytes)
{
...
postaction:
		...
		if (mem) {
			...
			ms->mem_used += chunksize(p);
			if (ms->mem_used > ms->mem_used_peak)
				ms->mem_used_peak = ms->mem_used;
		}
#endif
		POSTACTION(ms);
		return mem;
...
#ifdef KERNEL_EXTMEM_MSPACE
size_t mspace_mem_used(mspace msp)
{
	size_t result = 0;
	mstate ms = (mstate)msp;
...
size_t mspace_mem_used_peak(mspace msp)
{
	size_t result = 0;
	mstate ms = (mstate)msp;

	if (ok_magic(ms))
		result = ms->mem_used_peak;
	else
		USAGE_ERROR_ACTION(ms, ms);
	return result;
}
