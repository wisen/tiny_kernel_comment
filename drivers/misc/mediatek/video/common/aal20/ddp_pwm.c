...
#else
#if 0/*defined()*/ || defined(CONFIG_ARCH_MT6797) || \
	0/*defined()*/ || defined(CONFIG_ARCH_ELBRUS)
...
#ifdef CONFIG_RLK_SCREEN_FLASH_SUPPORT
int lcd_level = 102;
int  flag_lcd_flash = 0;
#endif
...
int disp_pwm_set_backlight_cmdq(disp_pwm_id_t id, int level_1024, void *cmdq)
{
	...
	{
		//PWM_ERR("[disp_pwm_set_backlight_cmdq] flag_lcd_flash = %d", flag_lcd_flash);
	}
#endif
	//PWM_ERR("[disp_pwm_set_backlight_cmdq] level_1024 = %d", level_1024);
	if ((DISP_PWM_ALL & id) == 0) {
...
//yang.li3 add start
#ifdef CONFIG_RLK_SCREEN_FLASH_SUPPORT
int disp_pwm_set_backlight_cmdq_flash(disp_pwm_id_t id, int level_1024, void *cmdq)
{
	unsigned long reg_base;
	int old_pwm;
	int index;
 ...
 int disp_pwm_set_backlight_flash( int level_1024)
 {
	if(level_1024>0)
	{
	 	disp_pwm_set_backlight_cmdq_flash( 0x01,  1023, NULL);
	}
	else
	{
		disp_pwm_set_backlight_cmdq_flash( 0x01,  lcd_level, NULL);
		PWM_ERR("[disp_pwm_set_backlight_flash] lcd_level = %d", lcd_level);
	}
