...
#define 	F16		KEY_F16 //RIGHT
#define 	F17	    KEY_F17 //DOWN
#define 	F18		KEY_F18 //LEFT
...
//	printk("===zyp gf_probe start \n");
    ...
    gf_dbg("[%s-%d]: chip_id_hi = 0x%x,chip_id_low = 0x%x, g_chipID is 0x%08x \n",__func__,__LINE__,chip_id_2,chip_id_1,g_chipID);
	if(chip_id_2 == 0x0022 && chip_id_1 == 0x02a0 ){
		gf_dbg("[%s-%d]check ID ok\n",__func__,__LINE__);
		app_get_fingerprint_name("GF368M");
	}
	else{
...
//	printk("===zyp gf_init \n");
    ...
    if (status < 0) {
//        printk("===zyp Failed to register char device!\n");
        FUNC_EXIT();
        return status;
    }
