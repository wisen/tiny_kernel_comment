...
static struct ges_init_info glghub_init_info;
...
static int glghub_local_init(void)
{
	...
	err = ges_register_control_path(&ctl, ID_GLANCE_GESTURE);
	if (err) {
	...
	err = ges_register_data_path(&data, ID_GLANCE_GESTURE);
	if (err) {
...
{
	ges_driver_add(&glghub_init_info, ID_GLANCE_GESTURE);
	return 0;
}
