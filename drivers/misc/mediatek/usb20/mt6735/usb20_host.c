...
//fmy modify 1214 start
#ifdef CONFIG_E31
extern int sm5414_int_number;
extern  int sm5414_irq_registration(void);
extern irqreturn_t sm5414_irq_handler(void);
#endif
//fmy modify 1214 end
u32 delay_time = 15;
...
void mt_usb_set_vbus(struct musb *musb, int is_on)
{
	...
	if (is_on) {
		...
		bq24259_set_otg_config(0x1); /* OTG */
		bq24259_set_boostv(0x7); /* boost voltage 4.998V */
		bq24259_set_boost_lim(0x1); /* 1.5A on VBUS */
...
static irqreturn_t mt_usb_ext_iddig_int(int irq, void *dev_id)
{
	//fmy modify 1214 start
	#ifdef CONFIG_E31
	//enable SM5414 INT interrupt when plug in host OTG cable
...
//fmy modify 0519 end
	...
	//fmy modify 0519 start
	#ifdef CONFIG_E31
	//request sm5414 INT interrupt when otg_int_init() 
	printk("fmy [%s]Line:%d \n",__FUNCTION__,__LINE__);
	retn = sm5414_irq_registration();
	if (retn < 0) {
		printk("fmy Error : can't initialize SM5414 irq\n");
	}
	#endif
	//fmy modify 0519 end
