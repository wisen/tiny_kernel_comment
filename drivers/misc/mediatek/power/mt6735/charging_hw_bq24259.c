...
/* ============================================================ // */
/* extern function */
extern void mt_power_off(void);		//add by wanglongchang
/* ============================================================ // */
...
static unsigned int charging_hw_init(void *data)
{
	...
	bq24259_set_vindpm(0x8);	/* VIN DPM check 4.52V */
	...
	if (batt_cust_data.high_battery_voltage_support)
		bq24259_set_vreg(0x38);	/* VREG 4.4V */
	else
	...
	if (KAL_TRUE == enable) {
		...
		if (mt_usb_is_device()) {
			...
			if (charging_get_error_state()) {
				battery_log(BAT_LOG_CRTI, "[charging_enable] bq24259_set_en_hiz(0x1)\n");
				bq24259_set_en_hiz(0x1);	/* disable power path */
			}
#if defined(CONFIG_USB_MTK_HDRC_HCD)
		}
#endif
...
static unsigned int charging_set_power_off(void *data)
{
	...
	mt_power_off();		//modify by wanglongchang
#endif
