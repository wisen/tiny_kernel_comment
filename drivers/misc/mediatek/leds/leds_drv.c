...
//add by wanghuijuan
#define BL_PLUS_EN_PIN (2 | 0x80000000)
static void bl_plus_set(int value)
{
    mt_set_gpio_mode(BL_PLUS_EN_PIN,GPIO_MODE_00);
    mt_set_gpio_dir(BL_PLUS_EN_PIN, GPIO_DIR_OUT);
    mt_set_gpio_out(BL_PLUS_EN_PIN, value);
    mdelay(5);
}

unsigned int value = 0;
static ssize_t bl_plus_show(struct device *dev , struct device_attribute *attr, char *buf )
{
    return 0;
}

static ssize_t bl_plus_store(struct device *dev , struct device_attribute *attr, const char *buf, size_t size)
{
    unsigned int value;
    sscanf(buf, "%d\n", &value);
    printk("value = %d\n", value);
...
static DEVICE_ATTR(bl_plus, 0664, bl_plus_show, bl_plus_store);
//add end
	...
	for (i = 0; i < MT65XX_LED_TYPE_TOTAL; i++) {
		...
		//add by wanghuijuan start
                if (strcmp(g_leds_data[i]->cdev.name, "lcd-backlight") == 0) {
                    ...
                    if (ret) {
                        LEDS_DRV_DEBUG
                            ("device_create_file bl_plus fail!\n");
                    }
                }
		//add end
