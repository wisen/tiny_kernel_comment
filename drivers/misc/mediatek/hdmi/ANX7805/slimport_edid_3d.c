...
{														\
	...
	SLIMPORT_TX_EDID_INFO(context->dev_context,				\
					"set %s\n",#x);	\
}
	...
	for (i = 0 ; i < sizeof(timing_modes_from_data_sheet)/sizeof(timing_modes_from_data_sheet[0]); ++i) {
		if (timing_modes_from_data_sheet[i].h_total == h_total) {
			if (timing_modes_from_data_sheet[i].v_total == v_total) {
				...
				SLIMPORT_TX_DBG_ERR(,"vic was 0, %s\n"
					,timing_modes_from_data_sheet[i].description);
				return ret_val;
			}
		}
	}
	SLIMPORT_TX_DBG_ERR(mhl_edid_3d_data->dev_context,
		...
		uint8_t VIC)
{
	...
	SLIMPORT_TX_EDID_READ(mhl_edid_3d_data->dev_context,
							  ...
							  P_18_byte_descriptor_u p_desc,uint8_t limit)
{
	...
	SLIMPORT_TX_EDID_INFO(mhl_edid_3d_data->dev_context,"limit: %d\n",(uint16_t)limit);
	if (limit) {
		for (i = 0 ; i < limit -1 ; ++i) {
...
// TODO: FD, TBI
		...
		*p_is_timing = 1;
		...
		SLIMPORT_TX_EDID_INFO(mhl_edid_3d_data->dev_context, "%sInterlaced\n",p_desc->dtd.flags.interlaced ? "" : "Non-" );
...
static void si_mhl_tx_prune_edid(edid_3d_data_p mhl_edid_3d_data)
{
		...
		*/
        {
            ...
            uint8_t VIC;
                ...
                if (0 == VIC)
                {
                    ...
                    SLIMPORT_TX_EDID_INFO(mhl_edid_3d_data->dev_context,"vic: 0x%02x deletion_mask:0x%08x this_bit:0x%08x\n"
		...
		while ( p_3D_u.p_as_bytes < mhl_edid_3d_data->parse_data.p_3d_limit) {
			...
			if (0 == VIC) {
				...
				uint32_t this_bit;
                SLIMPORT_TX_EDID_INFO(mhl_edid_3d_data->dev_context,"2D vic order: 0x%02x deletion_mask:0x%08x\n"
                        ,_2D_VIC_order
                        ,deletion_mask
                        );
                ...
                SLIMPORT_TX_EDID_INFO(mhl_edid_3d_data->dev_context,"2D vic order: 0x%02x this_bit:0x%08x\n"
                        ,_2D_VIC_order
                        ,this_bit
                        );
        ...
        SLIMPORT_TX_EDID_INFO(mhl_edid_3d_data->dev_context,"num_3D_structure_bytes_pruned:0x%x "
                        "byte14: 0x%02x "
                        "offset to DTDs: 0x%x "
                        "vsdb header: 0x%x\n"
    ...
    if (mhl_edid_3d_data->parse_data.p_byte_13_through_byte_15) {
	    ...
	    if (mhl_edid_3d_data->parse_data.p_HDMI_vsdb){
		    ...
		    SLIMPORT_TX_DBG_ERR(,"How can you have HDMI vics to prune if you have no HDMI VSDB?\n");	
	    }
        }
    }
	...
	)
{
	...
	SLIMPORT_TX_EDID_INFO(mhl_edid_3d_data->dev_context,"burstEntryCount3D_VIC: %d\n",mhl_edid_3d_data->parse_data.burst_entry_count_3d_vic);
...
#define DUMP_OFFSET(c,s,m) SLIMPORT_TX_EDID_INFO(c->dev_context, STRINGIZE(m)" offset:%x\n",SII_OFFSETOF(s,m) );
...
if (p_EDID_block_0->group.et##width##x##height##_##refresh##Hz##progressive) { \
	SLIMPORT_TX_EDID_INFO(context->dev_context, STRINGIZE(group)"."STRINGIZE(width)"x"STRINGIZE(height)"_"STRINGIZE(refresh)"Hz"STRINGIZE(progressive)"\n"); \
...
static void si_mhl_tx_parse_standard_timing(edid_3d_data_p mhl_edid_3d_data,PEDID_block0_t p_EDID_block_0)
{
	...
	SLIMPORT_TX_EDID_INFO(mhl_edid_3d_data->dev_context, "Parsing Standard Timing:\n");
	SLIMPORT_TX_EDID_INFO(mhl_edid_3d_data->dev_context, "========================\n");
...
void SiiMhlTxMakeItDVI(edid_3d_data_p mhl_edid_3d_data,PEDID_block0_t p_EDID_block_0)
{
	...
	mhl_edid_3d_data->parse_data.HDMI_sink = false;
	{
		...
		SLIMPORT_TX_DBG_INFO(mhl_edid_3d_data->dev_context,
		...
		if (EDID_REV_THREE != p_CEA_extension->revision) {
            ...
            uint8_t data_block_length;
				...
				case DBTC_VIDEO_DATA_BLOCK:
					SLIMPORT_TX_EDID_INFO(mhl_edid_3d_data->dev_context, "DBTC_VIDEO_DATA_BLOCK:\n");
				...
				case DBTC_AUDIO_DATA_BLOCK:
					{
						...
						SLIMPORT_TX_EDID_INFO(mhl_edid_3d_data->dev_context, "DBTC_AUDIO_DATA_BLOCK:\n");
						...
						pr_info("\n(uint8_t)p_audio_data_block->short_audio_descriptors[%d].max_channels_minus_one: %d\n",i,(uint8_t)p_audio_data_block->short_audio_descriptors[i].max_channels_minus_one);
							
						AudioFormatCode =p_audio_data_block->short_audio_descriptors[i].audio_format_code;
						pr_info("\nAudio Format Code %i\n",(int)AudioFormatCode);
						switch (AudioFormatCode){
							case 1: pr_info("Liniar PCM\n"); break;
							case 2: pr_info("AC-3\n"); break;
							case 3: pr_info("MPEG-1\n"); break;
							case 4: pr_info("MP3\n"); break;
							case 5: pr_info("MPEG2\n"); break;
							case 6: pr_info("ACC\n"); break;
							case 7: pr_info("DTS\n"); break;
							case 8: pr_info("ATRAC\n"); break;
...
/*#endif*/				    
			    ...
			    SLIMPORT_TX_EDID_INFO(mhl_edid_3d_data->dev_context,"check VS IEEE code failed!\n");
			    SLIMPORT_TX_EDID_INFO(mhl_edid_3d_data->dev_context,"00:%02X, 01:%02X, 02:%02X\n", p_vsdb->IEEE_OUI[0], p_vsdb->IEEE_OUI[1],p_vsdb->IEEE_OUI[2]);
			    mhl_edid_3d_data->parse_data.HDMI_sink = false;
		    }
...
static uint8_t parse_861_block(edid_3d_data_p mhl_edid_3d_data,uint8_t *p_EDID_block_data)
{
	...
	SLIMPORT_TX_EDID_INFO(mhl_edid_3d_data->dev_context,"tag:place holder EDID block:%p\n",p_EDID_block_data);			
	if (EDID_EXTENSION_BLOCK_MAP == p_CEA_extension->tag) {
		Pblock_map_t p_block_map = (Pblock_map_t)p_EDID_block_data;
		int i;
		SLIMPORT_TX_EDID_INFO(mhl_edid_3d_data->dev_context,"Edid: Block Map\n");
		...
		for(i=0;i<mhl_edid_3d_data->parse_data.num_EDID_extensions-1;++i) {
			if (EDID_EXTENSION_TAG != p_block_map->block_tags[i]) {
				SLIMPORT_TX_EDID_INFO(mhl_edid_3d_data->dev_context,"Edid: Adjusting number of extensions according to Block Map\n");
...
void si_mhl_tx_handle_atomic_hw_edid_read_complete(edid_3d_data_p mhl_edid_3d_data)
{
	PEDID_block0_t p_EDID_block_0 = (PEDID_block0_t)&mhl_edid_3d_data->EDID_block_data[0];
	uint8_t counter;
	SLIMPORT_TX_EDID_INFO(mhl_edid_3d_data->dev_context,"tag: Entire EDID Read complete\n");
	...
	if (0 == p_EDID_block_0->extension_flag) {
		...
		SLIMPORT_TX_EDID_INFO(mhl_edid_3d_data->dev_context," tag:place holder \n");			
		...
		for (counter = 1; counter <= mhl_edid_3d_data->parse_data.num_EDID_extensions;++counter) {
			SLIMPORT_TX_EDID_INFO(mhl_edid_3d_data->dev_context," counter:%d tag:place holder EDID block:%p\n",counter,&mhl_edid_3d_data->EDID_block_data[EDID_BLOCK_SIZE * counter]);			
...
void si_mhl_tx_initiate_edid_sequence(void *context)
{
	...
	SLIMPORT_TX_EDID_INFO(mhl_edid_3d_data->dev_context,"tag:\n");
...
int si_mhl_tx_get_num_cea_861_extensions(void *context, uint8_t block_number)
{
	...
	SLIMPORT_TX_EDID_INFO(mhl_edid_3d_data->dev_context,"block number:%d pb_data:%x\n",block_number,pb_data);
	...
	if (ne_SUCCESS == si_mhl_tx_drv_get_edid_fifo_next_block(mhl_edid_3d_data->drv_context, pb_data)) {
		if (0 == block_number) {
			if (!si_mhl_tx_check_edid_header(mhl_edid_3d_data,p_EDID_block_0)) {
				SLIMPORT_TX_DBG_ERR(mhl_edid_3d_data->dev_context,"EDID -> Incorrect Header pb_data:%x\n",pb_data);
...
//					return ne_BAD_HEADER_OFFSET_BY_1;
		...
		if (! si_mhl_tx_do_edid_checksum(pb_data)) {
			SLIMPORT_TX_DBG_ERR(mhl_edid_3d_data->dev_context,"EDID -> Checksum Error pb_data:%x\n",pb_data);
