...
struct maghub_ipi_data {
	...
	struct data_unit_t o_data_t;
	struct data_unit_t m_data_t;
};

static int maghub_o_setPowerMode(bool enable)
{
	int res = 0;
	struct maghub_ipi_data *obj = mag_ipi_data;

	MAGN_LOG("orientation enable value = %d\n", enable);
	if (!atomic_read(&obj->scp_init_done)) {
		MAGN_ERR("sensor hub has not been ready!!\n");
		return -1;
	}
	if (atomic_read(&obj->scp_init_done)) {
		res = sensor_enable_to_hub(ID_ORIENTATION, enable);
		if (res < 0)
			MAGN_ERR("maghub_o_setPowerMode is failed!!\n");
...
static int maghub_GetOData(char *buf, int size)
{
	struct maghub_ipi_data *obj = mag_ipi_data;
	struct data_unit_t data;
	uint64_t time_stamp = 0;
	uint64_t time_stamp_gpt = 0;
	int mag_o[MAGHUB_AXES_NUM];
	int err = 0;
	unsigned int status = 0;
	...
	time_stamp				= data.time_stamp;
	time_stamp_gpt			= data.time_stamp_gpt;
	mag_o[MAGHUB_AXIS_X]	= data.magnetic_t.x;
	mag_o[MAGHUB_AXIS_Y]	= data.magnetic_t.y;
	mag_o[MAGHUB_AXIS_Z]	= data.magnetic_t.z;
	status					= data.magnetic_t.status;

	MAGN_LOG("recv ipi: timestamp: %lld, timestamp_gpt: %lld, x: %d, y: %d, z: %d!\n", time_stamp, time_stamp_gpt,
		mag_o[MAGHUB_AXIS_X], mag_o[MAGHUB_AXIS_Y], mag_o[MAGHUB_AXIS_Z]);

	sprintf(buf, "%04x %04x %04x %04x", mag_o[MAGHUB_AXIS_X], mag_o[MAGHUB_AXIS_Y], mag_o[MAGHUB_AXIS_Z], status);

	if (atomic_read(&obj->trace) & MAG_ODATA_DEBUG)
		MAGN_ERR("RAW DATA: %s!\n", buf);
...
static int mag_recv_interrupt_ipidata(void *data, unsigned int len)
{
	struct maghub_ipi_data *obj = mag_ipi_data;
	SCP_SENSOR_HUB_DATA_P rsp = (SCP_SENSOR_HUB_DATA_P)data;

	MAGN_ERR("len = %d, type = %d, action = %d, errCode = %d\n", len, rsp->rsp.sensorType, rsp->rsp.action,
		rsp->rsp.errCode);
	...
	case SENSOR_HUB_NOTIFY:
		...
		case SCP_NOTIFY:
			...
			break;
		default:
			MAGN_ERR("Error sensor hub notify");
			break;
		}
		break;
	default:
		MAGN_ERR("Error sensor hub action");
		break;
	}
...
static int maghub_m_enable(int en)
{
	int res = 0;
...
static int maghub_m_set_delay(u64 ns)
{
	int delayms = 0, err = 0;
	struct maghub_ipi_data *obj = mag_ipi_data;
...
static int maghub_m_open_report_data(int open)
{
	return 0;
}

static int maghub_m_get_data(int *x, int *y, int *z, int *status)
{
...
static int maghub_o_enable(int en)
{
	int res = 0;

	res = maghub_o_setPowerMode(en);
	if (res)
		MAGN_ERR("maghub_o_enable is failed!!\n");
	return res;
}

static int maghub_o_set_delay(u64 ns)
{
	int delayms = 0, err = 0;
	struct maghub_ipi_data *obj = mag_ipi_data;
	...
	err = sensor_set_delay_to_hub(ID_ORIENTATION, delayms);
	if (err < 0) {
		MAGN_ERR("maghub_o_set_delay fail!\n");
		return err;
	}

	MAGN_LOG("maghub_o_set_delay (%d)\n", delayms);

	return err;
}

static int maghub_o_open_report_data(int open)
{
	return 0;
}

static int maghub_o_get_data(int *x, int *y, int *z, int *status)
{
	int err = 0;
	char buff[MAGHUB_BUFSIZE];
...
static int maghub_probe(struct platform_device *pdev)
{
	...
	err = SCP_sensorHub_rsp_registration(ID_MAGNETIC, mag_recv_interrupt_ipidata);
	if (err < 0) {
		MAGN_ERR("SCP_sensorHub_rsp_registration failed\n");
		goto exit_kfree;
	}
	...
	ctl.m_enable = maghub_m_enable;
	ctl.m_set_delay = maghub_m_set_delay;
	ctl.m_open_report_data = maghub_m_open_report_data;
	...
	mag_data.get_data_o = maghub_o_get_data;
	mag_data.get_data_m = maghub_m_get_data;
