...
bool gyro_success_Flag = false;
EXPORT_SYMBOL(gyro_success_Flag);
...
int gyro_driver_add(struct gyro_init_info *obj)
{
	...
	if (gyro_success_Flag == false) {
		if (gyro_probe()) {
			GYRO_ERR("failed to register gyro driver\n");
			return -ENODEV;
		}
		gyro_success_Flag = true;
	}
...
exit_alloc_input_dev_failed:
	destroy_workqueue(gyro_context_obj->gyro_workqueue);
	del_timer(&gyro_context_obj->timer);
	kfree(gyro_context_obj);
	gyro_context_obj = NULL;
