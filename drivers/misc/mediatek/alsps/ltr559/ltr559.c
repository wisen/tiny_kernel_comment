...
//added by peixuan.qiu 20151229 start
#define CONFIG_CUSTOM_SATURATION
#define CONFIG_CUSTOM_RLKINFO
#define CONFIG_CUSTOM_TP_GET_PSVAL

#ifdef CONFIG_CUSTOM_TP_GET_PSVAL
extern int (*get_psval)(void);
static int ltr559_get_psval(void);
#endif
//added by peixuan.qiu 20151229 end
...
/*----------------------------------------------------------------------------*/
#if defined(CONFIG_CUSTOM_RLKINFO)
...
#if defined(CONFIG_CUSTOM_RLKINFO)
	app_get_als_ps_sensor_name("ltr559"); 
#endif
...
static int ltr559_get_psval(void)
{
	int ret = 0;
	int psval, status;
	ret = ps_enable_nodata(1);
	if(ret) {
		APS_ERR("enable ps fail \n");
		goto EXIT;
	}
	ret = ps_get_data(&psval, &status);
	if(ret != 0) {
		 APS_ERR("read ps fail: %d\n", psval);
		goto EXIT;
	}
    if(!test_bit(CMC_BIT_PS,&ltr559_obj->enable)) {
	 ret = ps_enable_nodata(0);
	 if(ret) {
	       APS_ERR("enable ps fail \n");
	 	   goto EXIT;
	 }
    }
    printk("psval state %s \n",psval ? "far":"close");
    //check close or far
    if(!psval)
    {
        //close
