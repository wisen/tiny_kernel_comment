...
static int ps_recv_interrupt_ipidata(void *data, unsigned int len)
{
	struct alspshub_ipi_data *obj = obj_ipi_data;
...
static int alspshub_probe(struct platform_device *pdev)
{
	...
	err = SCP_sensorHub_rsp_registration(ID_PROXIMITY, ps_recv_interrupt_ipidata);
	if (err < 0) {
		APS_ERR("SCP_sensorHub_rsp_registration failed\n");
		goto exit_kfree;
	}
