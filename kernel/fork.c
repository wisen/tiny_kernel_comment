...
#endif
 //wisen: 6个slab cache,在proc_caches_init(start_kernel的时候调用)初始化
...
//wisen: files_cachep使用场景: fork进程的时候copy_files->dup_fd
/* SLAB cache for fs_struct structures (tsk->fs) */
struct kmem_cache *fs_cachep;
//wisen: fs_cachep使用场景: 同样是fork进程的时候copy_fs->copy_fs_struct
/* SLAB cache for vm_area_struct structures */
struct kmem_cache *vm_area_cachep;
//wisen: mm_cachep使用场景: 同样是fork进程的时候copy_mm->dup_mm->dup_mmap
//wisen: mm_cachep还有一个场景就是mmap
/* SLAB cache for mm_struct structures (tsk->mm) */
static struct kmem_cache *mm_cachep;
//wisen: mm_cachep使用场景: 同样是fork进程的时候copy_mm->dup_mm->allocate_mm
...
{
	mm->pgd = pgd_alloc(mm);//wisen: alloc新的PDG
	if (unlikely(!mm->pgd))
		return -ENOMEM;
	return 0;
}
...
static struct mm_struct *mm_init(struct mm_struct *mm, struct task_struct *p)
{
	...
	if (current->mm) {//wisen: 保留父进程的一些flag
	...
	if (mm_alloc_pgd(mm))//wisen: 重新分配页表
		goto fail_nopgd;
...
static struct mm_struct *dup_mm(struct task_struct *tsk)
{
	...
	if (!mm_init(mm, tsk))  
	//wisen: 在mm_init中将新进程的mmap置空，但是保留的父进程的一些flag，重新分配了页表
		goto fail_nomem;
...
static int copy_mm(unsigned long clone_flags, struct task_struct *tsk)
{
	...
	if (clone_flags & CLONE_VM) {             //wisen: 如果是克隆
		atomic_inc(&oldmm->mm_users); //wisen: 首先增加父进程的mm_users的引用计数
		mm = oldmm;                            //wisen: 将当前线程的mm指向父进程的mm,所以说线程和它的父进程之间是共享地址空间的
		goto good_mm;
	}

	retval = -ENOMEM;
	mm = dup_mm(tsk); //wisen: 如果不是克隆, 就创建一个新的mm,那么地址空间就是新的
	if (!mm)
		goto fail_nomem;
...
#if 1//#ifdef CONFIG_CGROUPS//CONFIG_CGROUPS=y
					...
					int trace)
{
	...
	retval = copy_mm(clone_flags, p);//wisen: 在copy_mm看看fork和clone的区别
	if (retval)
	...
	if (pid != &init_struct_pid) {
		retval = -ENOMEM;
		pid = alloc_pid(p->nsproxy->pid_ns_for_children);//wisen: 分配pid
		if (!pid)
			goto bad_fork_cleanup_io;
	}
 ...
 */
 //wisen: userspace的fork和pthread_create->clone其实最终在kernel space都是调用的do_fork
 //wisen: 只不过2者的参数不一样：
 //wisen: fork-->do_fork(SIGCHLD, 0, 0, NULL, NULL);
