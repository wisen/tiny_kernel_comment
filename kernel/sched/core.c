...
#include "../smpboot.h"
#if 1//#ifdef CONFIG_MTPROF//CONFIG_MTPROF=y
#include "mt_sched_mon.h"
#endif
#define CREATE_TRACE_POINTS
#include <trace/events/sched.h>

//wisen add for dump the backtrace of Uninterruptible sleep start
...
#if 1/*defined()*/ && defined(TIF_POLLING_NRFLAG)
/*
 ...
 */
#if 0/*defined()*/ || defined(CONFIG_PARAVIRT_TIME_ACCOUNTING)
...
#if 1//#ifdef CONFIG_LOCKDEP//CONFIG_LOCKDEP=y
	/*
...
#if defined(CONFIG_64BIT) && 1/*defined()*/
	/*
...
#if 1/*defined()*/ && (defined(CONFIG_DEBUG_PREEMPT) || \
...
need_resched:
	...
	if (sched_feat(HRTICK))
		hrtick_clear(rq);
#if 1/*defined(CONFIG_MT_SCHED_MONITOR)*/ && 1/*defined(CONFIG_MTPROF)*/
	...
	if (likely(prev != next)) {
		...
		++*switch_count;
        //wisen add for dump the backtrace of Uninterruptible sleep start
		if (strstr(prev->comm,"android.dialer")) {
			ktime_ns  = sched_clock();
			curtask_state = prev->state;
		}		
	//wisen add for dump the backtrace of Uninterruptible sleep end
		...
		//wisen: 某个进程如果进入uninterrupt sleep超过1ms，那么它重新被schedule开始运行的时候会被打印当前的backtrace
		if (strstr(rq->curr->comm, "android.dialer")) {
			unsigned long long tt = sched_clock() - ktime_ns;
			
			if ((tt > 1000000) && (curtask_state & TASK_UNINTERRUPTIBLE)) {
				//show_stack(rq->curr, NULL);
				int __i, j, length;
				struct stack_trace trace;
				unsigned long entries[32] = {0};
				char buf[300];

				printk("Q Q > %s over %llu ns, state 0x%08lx\n", rq->curr->comm, tt, curtask_state);

				trace.entries = entries;
				trace.nr_entries = 0;
				trace.max_entries = 32;
				trace.skip = 0;
				save_stack_trace_tsk(rq->curr, &trace);

				for (__i = 2; __i <= 16; __i+=4) {
					if (!entries[__i])
						break;
					...
					for (j = 0; j < 4; j++) {
						...
						length += snprintf(&buf[length], 300, "%pS", (void *)(entries[j+__i]));
					 	buf[length-10] = ' ';
						buf[length- 9] = '/';
						buf[length- 8] = '\0';
						length -= 8;
...
asmlinkage __visible void __sched schedule(void)
{
	...
	//wisen: sched_submit_work主要是来避免死锁。
...
//wisen: 这里其实有个东西要考虑到，我们平时看systrace的时候，会看到uninterrupt sleep很长，都说是被IO block了，说IO慢
//这句话本身也没有错，但是唯一要考虑的时候不能把所有的锅都让io来背，为什么？从io_schedule可以看出，在做io的时候先把
//in_iowait设置为1，然后进程被调度出去了,并且只有当
	current->in_iowait = 1;
	schedule();
	current->in_iowait = 0;
...
#if defined(CONFIG_SCHED_DEBUG) && 1/*defined()*/
