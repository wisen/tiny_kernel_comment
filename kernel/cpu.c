...
static DECLARE_BITMAP(cpu_possible_bits, 8/*CONFIG_NR_CPUS=8*/) __read_mostly
	= CPU_BITS_ALL;
#else
static DECLARE_BITMAP(cpu_possible_bits, 8/*CONFIG_NR_CPUS=8*/) __read_mostly;
#endif
...
static DECLARE_BITMAP(cpu_online_bits, 8/*CONFIG_NR_CPUS=8*/) __read_mostly;
const struct cpumask *const cpu_online_mask = to_cpumask(cpu_online_bits);
EXPORT_SYMBOL(cpu_online_mask);

static DECLARE_BITMAP(cpu_present_bits, 8/*CONFIG_NR_CPUS=8*/) __read_mostly;
const struct cpumask *const cpu_present_mask = to_cpumask(cpu_present_bits);
EXPORT_SYMBOL(cpu_present_mask);

static DECLARE_BITMAP(cpu_active_bits, 8/*CONFIG_NR_CPUS=8*/) __read_mostly;
