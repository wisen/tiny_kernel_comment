...
ssize_t new_sync_read(struct file *filp, char __user *buf, size_t len, loff_t *ppos)
{
	...
	ssize_t ret;
	//这里的iov_iter接口可以参考解释: https://lwn.net/Articles/625077/
...
ssize_t vfs_read(struct file *file, char __user *buf, size_t count, loff_t *pos)
{
	...
	if (!(file->f_mode & FMODE_CAN_READ))
		return -EINVAL;
	//这里有个细节，就是这个传进来的buf一定要是user space的地址,不然会访问报错，这里通过的是access_ok来检查的
...
ssize_t vfs_write(struct file *file, const char __user *buf, size_t count, loff_t *pos)
{
	...
	if (ret >= 0) {
		...
		if (file->f_op->write)
			//这里是具体的文件系统的file_operations, 比如 f2fs_file_operations
			//具体都在 f2fs_iget 中初始化 inode->i_fop = &f2fs_file_operations;
